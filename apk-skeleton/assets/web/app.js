...
const __koeCacheTTL={get_friends_list:20000,get_chats:5000,get_user_posts:1500,get_user_settings:15000,get_followees:4000,get_followers:4000,get_notifications:1500,get_gift_history:8000,get_call_records:8000,get_feed_post:3000,get_timeline_comments:3000,get_timeline_likers:5000,get_my_profile:1e4,get_badges:4000,get_account_balance:4000,get_room_history:5000,get_activity_heatmap:5000,get_moderation_settings:8000,get_official_links:120000,get_community_categories:120000,get_my_communities:6000};
...
function koeErrMsg(r,ctx){
  try{
    ...
    if(vs&&vs!==-999){
      var VS={1:"サーバーに拒否されました。",101:"認証に失敗しました。",102:"ログインの有効期限が切れました。もう一度ログインしてください。",119:"このアカウントは利用停止されています。",
        201:"入力が空です。",202:"文字数が制限を超えています。",203:"すでに登録されています。",205:"使用できない言葉が含まれています。表現を変えてお試しください。",
        401:"上限に達しています。",402:"相手が見つかりませんでした。退会・削除された可能性があります。",801:"アプリのバージョンが古いと判定されました。",802:"メンテナンス中です。",901:"対象が見つかりませんでした。削除された可能性があります。"};
...
async function koeLoadDiscover(kind){
  var box=document.getElementById("koeDiscoverList"); if(!box) return;
  document.querySelectorAll(".koe-disc-chip").forEach(function(c){c.classList.toggle("active",c.dataset.disc===kind)});
  box.innerHTML=(typeof skeletonCards==="function")?skeletonCards(3):"読み込み中…";
  var m = kind==="recommended"?"get_recommended_users":kind==="birthday"?"get_birthday_users":kind==="requests"?"get_follow_requests":"get_hima_users";
  var r = await callApi(m, "1");
  if(!r||r.ok===false){ box.innerHTML='<div class="empty-msg">'+escapeHtml(koeErrMsg(r))+'</div>'; return; }
  var us = r.users||r.list||[];
  if(!us.length){ box.innerHTML='<div class="empty-msg">'+(kind==="requests"?"フォロー申請はありません":"該当するユーザーはいません")+'</div>'; return; }
  box.innerHTML = us.map(function(u){
    var sub = [u.age?u.age+"歳":"", u.area||"", u.login_status_with_unit||u.login_status||""].filter(Boolean).join(" ・ ");
    return '<div class="card" onclick="viewProfile('+(Number(u.user_id)||0)+')">'+avatarHtml(u.name,u.icon_url)+
      '<div class="card-body"><div class="card-name">'+escapeHtml(u.name||("user "+u.user_id))+' <span class="uid-tag">ID:'+u.user_id+'</span></div>'+
      (sub?'<div class="card-sub">'+escapeHtml(sub)+'</div>':'')+
      (u.comment?'<div class="card-sub">'+escapeHtml(String(u.comment).slice(0,60))+'</div>':'')+'</div></div>';
  }).join("");
}
(function(){
  function bind(){
    document.querySelectorAll(".koe-disc-chip").forEach(function(c){
      ...
      c.addEventListener("click",function(){ koeLoadDiscover(c.dataset.disc); try{sfx("tab")}catch(e){} });
    });
    var d=document.getElementById("koeDiscoverSection");
    if(d&&!d.__b){ d.__b=1; d.addEventListener("toggle",function(){ if(this.open&&!document.getElementById("koeDiscoverList").innerHTML.trim()) koeLoadDiscover("hima"); }); }
    var stg=document.getElementById("koeStorageSection");
    if(stg&&!stg.__b){ stg.__b=1; stg.addEventListener("toggle",function(){ if(this.open) koeRenderStorage(); }); }
    var lm=document.getElementById("koeLimitSection");
    if(lm&&!lm.__b){ lm.__b=1; lm.addEventListener("toggle",function(){ if(this.open) koeRenderLimitLog(); }); }
    var jb=document.getElementById("joinOtherBtn");
    if(jb&&!jb.__b){ jb.__b=1; jb.addEventListener("click",function(){ if(typeof joinOtherRoom==="function") joinOtherRoom(); }); }
  }
  document.addEventListener("DOMContentLoaded",bind); setTimeout(bind,800); setTimeout(bind,2500);
})();

// ===== DMの音声送信 =====
var __dmRec=null,__dmChunks=[],__dmData=null,__dmExt="webm",__dmType="audio/webm",__dmStart=0,__dmSec=0,__dmTimer=null;
function koeDmClearVoice(){__dmData=null;__dmSec=0;var r=document.getElementById("chatVoiceRow"),a=document.getElementById("chatVoicePreview");if(r)r.style.display="none";if(a)a.src="";var b=document.getElementById("chatVoiceBtn");if(b)b.classList.remove("recording")}
async function koeDmToggleVoice(){
  var btn=document.getElementById("chatVoiceBtn");
  if(__dmRec&&__dmRec.state==="recording"){__dmRec.stop();return}
  if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){toast("この端末では録音に対応していません","error");return}
  var stream;try{stream=await navigator.mediaDevices.getUserMedia({audio:true})}catch(e){toast("マイクの使用を許可してください","error");return}
  var mime=(typeof __pickVoiceMime==="function")?__pickVoiceMime():"";
  try{__dmRec=mime?new MediaRecorder(stream,{mimeType:mime}):new MediaRecorder(stream)}catch(e){try{__dmRec=new MediaRecorder(stream)}catch(e2){toast("録音を開始できませんでした","error");return}}
  __dmChunks=[];
  __dmRec.ondataavailable=function(e){if(e.data&&e.data.size)__dmChunks.push(e.data)};
  __dmRec.onstop=function(){
    try{stream.getTracks().forEach(function(t){t.stop()})}catch(e){}
    if(__dmTimer){clearInterval(__dmTimer);__dmTimer=null}
    var type=(__dmRec.mimeType||mime||"audio/webm").split(";")[0];
    __dmType=type;__dmExt=type.indexOf("mp4")>=0?"m4a":type.indexOf("ogg")>=0?"ogg":"webm";
    __dmSec=Math.max(1,Math.round((Date.now()-__dmStart)/1000));
    var blob=new Blob(__dmChunks,{type:type}),fr=new FileReader();
    fr.onload=function(){__dmData=fr.result;var r=document.getElementById("chatVoiceRow"),a=document.getElementById("chatVoicePreview");if(a)a.src=__dmData;if(r)r.style.display="flex"};
    fr.readAsDataURL(blob);
    if(btn)btn.classList.remove("recording");
  };
  __dmRec.start();__dmStart=Date.now();
  if(btn)btn.classList.add("recording");
  toast("録音中… もう一度タップで停止（最大90秒）");
  __dmTimer=setInterval(function(){ if((Date.now()-__dmStart)/1000>=90&&__dmRec&&__dmRec.state==="recording")__dmRec.stop(); },500);
}
async function koeDmSendVoice(){
  ...
  var r=await callApi("send_voice_message",currentChat.chatId,currentChat.targetId,__dmData,__dmExt,__dmType,String(__dmSec||0));
  if(btn)btn.disabled=false;
  if(r&&r.ok){koeDmClearVoice();try{sfx("send")}catch(e){}await reloadMessages(true);setTimeout(function(){reloadMessages(true)},1200);setTimeout(function(){reloadMessages(true)},3000);return true}
  toast(koeErrMsg(r),"error");return false;
}
(function(){
  function bind(){
    var b=document.getElementById("chatVoiceBtn");
    if(b&&!b.__b){b.__b=1;b.addEventListener("click",koeDmToggleVoice)}
    var c=document.getElementById("chatVoiceClear");
    if(c&&!c.__b){c.__b=1;c.addEventListener("click",koeDmClearVoice)}
  }
  document.addEventListener("DOMContentLoaded",bind);setTimeout(bind,800);setTimeout(bind,2500);
})();
...
async function koeRecording(action){
  ...
  if(!ch){ toast("通話に参加していません","error"); return; }
  var note=document.getElementById("callRecNote");
  if(note) note.textContent = action==="start"?"録音を開始しています…":"録音を停止しています…";
  if(action==="start"){ await callApi("recording_channel","join",ch); }
  var r=await callApi("recording_channel",action,ch);
  if(r&&r.ok){ if(note) note.textContent = action==="start"?"録音中です":"録音を停止しました"; toast(action==="start"?"録音を開始しました":"録音を停止しました"); }
  else { if(note) note.textContent="録音を操作できませんでした"; toast(koeErrMsg(r),"error"); }
}
(function(){
  function bind(){
    var s=document.getElementById("callRecStartBtn"), e=document.getElementById("callRecStopBtn");
    if(s&&!s.__b){s.__b=1;s.addEventListener("click",function(){koeRecording("start")})}
    if(e&&!e.__b){e.__b=1;e.addEventListener("click",function(){koeRecording("stop")})}
  }
  document.addEventListener("DOMContentLoaded",bind);setTimeout(bind,900);setTimeout(bind,2600);
})();

// ===== 運営アンケート =====
function koeEnqArr(o){ if(!o) return []; if(Array.isArray(o)) return o;
  var keys=["enquetes","questions","choices","data","items","list"];
  for(var i=0;i<keys.length;i++){ var v=o[keys[i]]; if(Array.isArray(v)) return v; if(v&&typeof v==="object"){var r=koeEnqArr(v); if(r.length) return r;} }
  return []; }
async function koeLoadEnquetes(){
  var box=document.getElementById("koeEnqueteList"); if(!box) return;
  box.innerHTML=skeletonCards(2);
  var r=await callApi("get_enquetes");
  if(!r||r.ok===false){ box.innerHTML='<div class="empty-msg">'+escapeHtml(koeErrMsg(r))+'</div>'; return; }
  var list=koeEnqArr(r.enquetes||r.body||r);
  if(!list.length){ box.innerHTML='<div class="empty-msg">いま回答できるアンケートはありません</div>'; return; }
  box.innerHTML=list.map(function(e,i){
    var id=e.id||e.enquete_id||i, ttl=e.title||e.name||e.description||("アンケート "+id);
    return '<div class="card" data-enq="'+escAttr(String(id))+'"><div class="card-body"><div class="card-name">'+escapeHtml(String(ttl))+'</div>'+
      (e.description&&e.title?'<div class="card-sub">'+escapeHtml(String(e.description).slice(0,80))+'</div>':'')+'</div></div>';
  }).join("");
  box.querySelectorAll("[data-enq]").forEach(function(c){ c.addEventListener("click",function(){ koeOpenEnquete(c.dataset.enq) }) });
}
async function koeOpenEnquete(id){
  var form=document.getElementById("koeEnqueteForm"); if(!form) return;
  form.style.display="block"; form.innerHTML="読み込み中…";
  var r=await callApi("get_enquete_questions",String(id));
  if(!r||r.ok===false){ form.innerHTML='<div class="empty-msg">'+escapeHtml(koeErrMsg(r))+'</div>'; return; }
  var qs=koeEnqArr(r.questions||r.body||r);
  if(!qs.length){ form.innerHTML='<div class="empty-msg">設問を取得できませんでした</div>'; return; }
  form.innerHTML=qs.map(function(q,qi){
    var qid=q.id||q.question_id||qi, txt=q.text||q.title||q.body||q.question||("設問 "+(qi+1));
    var ch=koeEnqArr(q.choices||q.enquete_choices||q.options||[]);
    return '<div class="card" style="display:block"><div class="card-name" style="margin-bottom:6px;">'+escapeHtml(String(txt))+'</div>'+
      (ch.length?ch.map(function(c,ci){ var cid=c.id||c.choice_id||ci, ct=c.text||c.title||c.body||("選択肢 "+(ci+1));
        return '<label class="check-row"><input type="radio" name="enq_'+escAttr(String(qid))+'" value="'+escAttr(String(cid))+'"> '+escapeHtml(String(ct))+'</label>'; }).join("")
        :'<input class="enq-free" data-q="'+escAttr(String(qid))+'" type="text" placeholder="自由回答">')+
      '</div>';
  ...
  form.querySelector("#koeEnqSubmit").addEventListener("click",async function(){
    var answers=[];
    form.querySelectorAll("input[type=radio]:checked").forEach(function(inp){ answers.push({question_id:inp.name.replace("enq_",""),choice_id:inp.value}) });
    form.querySelectorAll(".enq-free").forEach(function(inp){ if(inp.value.trim()) answers.push({question_id:inp.dataset.q,text:inp.value.trim()}) });
    if(!answers.length){ toast("回答を選んでください","error"); return; }
    this.disabled=true;
    var res=await callApi("send_enquete_answer_v2",JSON.stringify({enquete_id:id,answers:answers}));
    this.disabled=false;
    if(res&&res.ok){ toast("回答を送信しました"); try{ callApi("track_enquete",String(id),"1") }catch(e){} form.style.display="none"; form.innerHTML=""; }
...
(function(){
  function bind(){
    ...
    if(s&&!s.__b){ s.__b=1; s.addEventListener("toggle",function(){ if(this.open&&!document.getElementById("koeEnqueteList").innerHTML.trim()) koeLoadEnquetes(); }); }
  }
  document.addEventListener("DOMContentLoaded",bind);setTimeout(bind,900);setTimeout(bind,2600);
})();
...
(function(){
  if(window.__koeNotifWatch) return; window.__koeNotifWatch=true;
  var last=-1, t0=Date.now(), lastCheck=0;
  /* 操作中は5秒、しばらく放置していれば20秒間隔にする(電池と通信の節約)。
     アプリに戻った瞬間は下の visibilitychange で即確認するので、遅れは出ない。 */
  function gap(){ var idle=Date.now()-(window.__koeLastTouch||t0); if(idle<45000) return 8000; if(idle<180000) return 20000; return 45000; }
  try{ ["pointerdown","keydown","touchstart"].forEach(function(ev){
    document.addEventListener(ev,function(){ window.__koeLastTouch=Date.now() },{passive:true,capture:true}) }) }catch(e){}
  setInterval(async function(){
    try{
      ...
      if(b){ if(c>0){b.textContent=c>99?"99+":String(c);b.style.display="block"} else b.style.display="none"; }
      if(last>=0&&c>last){
        try{ sfx("notify") }catch(e){}
        var act=document.querySelector(".page.active");
        if(act&&act.id==="page-notifications"&&typeof loadNotifications==="function") loadNotifications(typeof currentNotifKind!=="undefined"?currentNotifKind:"normal");
      }
      last=c;
    }catch(e){}
  },5000);
  ...
  document.addEventListener("visibilitychange",function(){
    if(!document.hidden){
      ...
      try{ callApi("get_unread_notif_count").then(function(r){
        var c=r&&(r.count!==undefined?r.count:r.unread_count);
        var b=document.getElementById("notifBadge");
        if(b&&typeof c==="number"){ if(c>0){b.textContent=c>99?"99+":String(c);b.style.display="block"} else b.style.display="none"; }
        var act=document.querySelector(".page.active");
        if(act&&act.id==="page-notifications"&&typeof loadNotifications==="function") loadNotifications(typeof currentNotifKind!=="undefined"?currentNotifKind:"normal");
...
function koeLimitNote(method, result){
  try{
    var msg = (result&&(result.message||""))+" "+((result&&result.raw)||"")+" "+((result&&result.body&&(result.body.detail||result.body.displayable_detail||result.body.message))||"");
    if(!/超過|制限|しばらく|多すぎ|上限|too many|rate/i.test(msg)) return;
    var prev = window.__koeLastOk[method]||0;
    var rec = {t:Date.now(), m:method, since: prev?Math.round((Date.now()-prev)/1000):null,
               msg:String(msg).replace(/\s+/g," ").trim().slice(0,140), status:(result&&result.status)||0};
    var log=[]; try{ log=JSON.parse(localStorage.getItem("koe_limit_log")||"[]") }catch(e){}
    log.unshift(rec); log=log.slice(0,30);
    try{ localStorage.setItem("koe_limit_log",JSON.stringify(log)) }catch(e){}
    try{ if(typeof toast==="function") toast("サーバーの制限にかかりました"+(rec.since!==null?"（前回成功から"+rec.since+"秒）":""),"error") }catch(e){}
    try{ koeRenderLimitLog() }catch(e){}
  }catch(e){}
}
function koeRenderLimitLog(){
  var box=document.getElementById("koeLimitLog"); if(!box) return;
  var log=[]; try{ log=JSON.parse(localStorage.getItem("koe_limit_log")||"[]") }catch(e){}
  if(!log.length){ box.innerHTML='<div class="empty-msg">まだ制限にかかった記録はありません</div>'; return; }
  var names={create_room:"枠の作成",create_feed_post:"つぶやく投稿",create_timeline_post:"話そう投稿",
    create_feed_post_with_image:"画像つき投稿",create_timeline_post_with_image:"画像つき投稿",
    reply_timeline_post:"返信",send_message:"DM送信",send_image_message:"DM画像",send_voice_message:"DM音声",
    toggle_timeline_like:"いいね",follow_user:"フォロー",moderation_report_spam:"BANリスト申請"};
  box.innerHTML=log.map(function(r){
    var d=new Date(r.t);
    return '<div class="card" style="display:block"><div class="card-name">'+(names[r.m]||r.m)+
      (r.since!==null?' <span class="uid-tag">前回成功から'+r.since+'秒</span>':'')+'</div>'+
      '<div class="card-sub">'+(d.getMonth()+1)+'/'+d.getDate()+' '+String(d.getHours()).padStart(2,"0")+':'+String(d.getMinutes()).padStart(2,"0")+
      ' ・ '+escapeHtml(r.msg)+'</div></div>';
  }).join("");
}
...
function koeStorageStats(){
  ...
  try{
    for(var i=0;i<localStorage.length;i++){
      ...
      var b=(k.length+v.length)*2; out.total+=b; out.items.push({k:k,b:b});
    }
    out.items.sort(function(a,b){return b.b-a.b});
  }catch(e){}
  return out;
}
function koeFmtBytes(b){ return b>=1048576?(b/1048576).toFixed(1)+"MB":b>=1024?Math.round(b/1024)+"KB":b+"B"; }
/* キャッシュは「使わない」方針。上限を小さくして、超えた分は即捨てる（設定やログイン情報は触らない） */
var KOE_STOR_CAPS={koe_deleted_posts:60,koe_post_cache:80,koe_seen_posts:500,koe_liked_posts:500,
  koe_search_hist:10,koe_notif_seen:300,koe_call_logs:60,koe_limit_log:40,koe_spam_reports:40,koe_bm_local:200};
function koeTrimStorage(){
  try{
    ["koe_tlc_all","koe_tlc_following"].forEach(function(k){
      var v=localStorage.getItem(k); if(v&&v.length>40000){ try{localStorage.removeItem(k)}catch(e){} }
    });
    var cap=function(k,n){ try{ var a=JSON.parse(localStorage.getItem(k)||"[]"); if(Array.isArray(a)&&a.length>n){ localStorage.setItem(k,JSON.stringify(a.slice(0,n))) } }catch(e){} };
    for(var k in KOE_STOR_CAPS){ if(Object.prototype.hasOwnProperty.call(KOE_STOR_CAPS,k)) cap(k,KOE_STOR_CAPS[k]); }
    /* 想定外に膨らんだ項目（1件で200KB超）は中身がキャッシュ系なら捨てる */
    try{
      var keep=/^(koe_accounts|koe_current_account|koe_autologin|koe_autologin_cred|koe_pin|koe_draft|koe_font|koe_font_custom|koe_fontsize|koe_bio|koe_room_defaults|koe_ngwords|koe_muted_users|koe_blocked_users)$/;
      for(var i=localStorage.length-1;i>=0;i--){
        var kk=localStorage.key(i); if(!kk||keep.test(kk)) continue;
        var vv=localStorage.getItem(kk)||""; if(vv.length>200000){ localStorage.removeItem(kk); }
      }
    }catch(e){}
    /* 端末側の一時ファイル（画像など）も上限を超えたら自動で削除 */
    try{ if(window.AndroidApi&&window.AndroidApi.trimAppCache) window.AndroidApi.trimAppCache(); }catch(e){}
  }catch(e){}
}
function koeRenderStorage(){
  var box=document.getElementById("koeStorageBox"); if(!box) return;
  var s=koeStorageStats();
  var names={koe_tlc_all:"タイムラインの表示キャッシュ",koe_tlc_following:"フォロー中の表示キャッシュ",
    koe_post_cache:"投稿の控え（削除検知用）",koe_deleted_posts:"削除された投稿の記録",
    koe_liked_posts:"いいね済みの記録",koe_seen_posts:"既読の記録",koe_blocked_users:"ブロック済みの記録",
    koe_act_data:"活動時間帯の集計",koe_limit_log:"制限にかかった記録",koe_spam_reports:"BAN申請の履歴"};
  var top=s.items.slice(0,8).map(function(x){
    return '<div class="card" style="display:block;padding:8px 10px"><div class="card-name" style="font-size:13px">'+escapeHtml(names[x.k]||x.k)+'</div><div class="card-sub">'+koeFmtBytes(x.b)+'</div></div>';
  }).join("");
  var app={cache:0,files:0};
  try{ if(window.AndroidApi&&window.AndroidApi.appStorageInfo){ var ai=JSON.parse(window.AndroidApi.appStorageInfo()||"{}"); if(ai&&ai.ok){app.cache=ai.cache||0;app.files=ai.files||0;} } }catch(e){}
  box.innerHTML='<div class="card-sub" style="margin-bottom:6px">いま使っている容量：一時ファイル（画像など）'+koeFmtBytes(app.cache)+' ／ 表示用の控え・記録 '+koeFmtBytes(s.total)+'</div>'+top+
  ...
  if(b1) b1.addEventListener("click",function(){ koeTrimStorage(); toast("キャッシュを整理しました"); koeRenderStorage(); });
  var ball=document.getElementById("koeStorClearAll");
  if(ball) ball.addEventListener("click",function(){
    var freed=0;
    try{ freed=koeStorageStats().total }catch(e){}
    try{ ["koe_tlc_all","koe_tlc_following","koe_post_cache","koe_seen_posts","koe_liked_posts","koe_notif_seen","koe_call_logs","koe_act_data","koe_search_hist"].forEach(function(k){localStorage.removeItem(k)}) }catch(e){}
    var nat=0;
    try{ if(window.AndroidApi&&window.AndroidApi.clearAppCache){ var r=JSON.parse(window.AndroidApi.clearAppCache()||"{}"); if(r&&r.ok) nat=r.freed||0; } }catch(e){}
    try{ freed=Math.max(0,freed-koeStorageStats().total) }catch(e){}
    toast("キャッシュを削除しました（"+koeFmtBytes(freed+nat)+"）");
    try{ sfx("success") }catch(e){}
    setTimeout(koeRenderStorage,300);
  });
}
try{ setTimeout(koeTrimStorage, 2000); setInterval(koeTrimStorage, 300000);
  document.addEventListener("visibilitychange",function(){ if(document.hidden) koeTrimStorage(); });
}catch(e){}
...
const isBg=/^(moderation_auto_spam|js_diag_log|system_arrival|send_registration_id|recording_channel)$/.test(methodName);
if(!isReadOnly&&!isBg){try{__koeCache.clear()}catch(e){}try{__koeInflight.clear()}catch(e){}}
const p=(async()=>{
/* 起動直後: 画面はキャッシュで先に出しておき、データ取得だけ認証の完了を待つ。
   ここで待つのは重要: 関数の先頭で待つと、同じ呼び出しの相乗り(重複排除)より前に中断してしまい
   起動時に同じAPIが2回走る。 */
if(window.__koeAuthGate&&!/^(login|signup|is_logged_in|logout|twitter_|get_native_log|js_diag_log)/.test(methodName)){
/* 万一解除されなくても通信が止まりっぱなしにならないよう、最大5秒で打ち切る */
try{await Promise.race([window.__koeAuthGate,new Promise(function(r){setTimeout(r,5000)})])}catch(e){}
_t0=performance.now();
}
...
try{ if(result&&typeof result==="object"){ if(result.ok===false){ result.__method=methodName; if(!result.friendly) result.friendly=koeErrMsg(result,methodName); koeLimitNote(methodName,result); } else if(!isReadOnly){ window.__koeLastOk[methodName]=Date.now(); } } }catch(e){}
...
const drop=()=>{setTimeout(()=>{if(__koeInflight.get(key)===p)__koeInflight.delete(key)},__shareAfter)};
...
function koeRenderFontList(q){
  ...
  KOE_FONT_GROUPS.forEach(function(g){
    var keys=Object.keys(KOE_FONTS).filter(function(k){
      var f=KOE_FONTS[k]; if((f.g||"normal")!==g.k) return false;
      if(!q) return true;
      return (f.label||k).toLowerCase().indexOf(q)>=0 || (f.note||"").toLowerCase().indexOf(q)>=0 || k.indexOf(q)>=0;
    });
    if(!keys.length) return;
    html+='<div class="ios-group-title">'+g.t+'</div><div class="ios-group">';
    keys.forEach(function(k){
      var f=KOE_FONTS[k];
      html+='<button type="button" class="ios-row'+(cur===k?" selected":"")+'" data-font="'+escAttr(k)+'">'+
        '<span class="ios-row-main"><span class="ios-row-name" style="font-family:'+escAttr(f.family||"inherit")+'">'+escapeHtml(f.label||k)+'</span>'+
        '<span class="ios-row-sample" style="font-family:'+escAttr(f.family||"inherit")+'">あいうえお 12345 Koe</span></span>'+
        '<span class="ios-row-note">'+escapeHtml(f.note||"")+'</span>'+
  ...
  try{
    var io=new IntersectionObserver(function(es){
      es.forEach(function(en){ if(en.isIntersecting){ var k=en.target.dataset.font; var f=KOE_FONTS[k]; if(f&&f.url) loadGFont(f.url); io.unobserve(en.target); } });
    },{root:null,rootMargin:"200px"});
    box.querySelectorAll(".ios-row").forEach(function(r){ io.observe(r) });
  }catch(e){ box.querySelectorAll(".ios-row").forEach(function(r){ var f=KOE_FONTS[r.dataset.font]; if(f&&f.url) loadGFont(f.url) }); }
  box.querySelectorAll(".ios-row").forEach(function(r){
    r.addEventListener("click",function(){
      var k=r.dataset.font;
      try{ localStorage.setItem("koe_font",k) }catch(e){}
      try{ localStorage.removeItem("koe_font_file"); localStorage.removeItem("koe_font_file_name"); }catch(e){}
       try{ if(window.koeClearUserFont) koeClearUserFont(); }catch(e){}
       applyFont();
      box.querySelectorAll(".ios-row").forEach(function(x){ x.classList.toggle("selected", x===r) });
      try{ sfx("select"); haptic(8) }catch(e){}
    });
  });
}
function koeSetFontFamily(fam){try{document.body.style.fontFamily=fam||"";document.documentElement.style.setProperty("--koe-font",fam||"")}catch(e){}}
function applyFont(){let key="system",custom="";try{key=localStorage.getItem("koe_font")||"system",custom=localStorage.getItem("koe_font_custom")||""}catch(e){}if("custom"===key&&custom.trim())return loadGFont(custom.trim().replace(/\s+/g,"+")),void koeSetFontFamily("'"+custom.trim()+"','Hiragino Sans','Noto Sans JP',sans-serif");const f=KOE_FONTS[key]||KOE_FONTS.system;f.url&&loadGFont(f.url),koeSetFontFamily(f.family||"")}const __loadedScripts={};function loadScript(src){return __loadedScripts[src]||(__loadedScripts[src]=new Promise((resolve,reject)=>{const el=document.createElement("script");el.src=src,el.onload=()=>resolve(),el.onerror=()=>{delete __loadedScripts[src],reject(new Error("load failed: "+src))},document.head.appendChild(el)})),__loadedScripts[src]}let __likedCache=null;function getLikedSet(){if(__likedCache)return __likedCache;try{__likedCache=new Set(JSON.parse(localStorage.getItem("koe_liked_posts")||"[]").map(String))}catch(e){__likedCache=new Set}return __likedCache}function markLiked(id,liked){const set=getLikedSet();liked?set.add(String(id)):set.delete(String(id));try{var arr=[...set];if(arr.length>3000)arr=arr.slice(arr.length-3000);__likedCache=new Set(arr);localStorage.setItem("koe_liked_posts",JSON.stringify(arr))}catch(e){}}function postLiked(p){try{return!!p.liked||getLikedSet().has(String(p.id))}catch(e){return!!p.liked}}function koeMethodIcon(m){
...
function koeInitZoom(){
  ...
  function zoomTo(nv,cx,cy,anim){
    ...
    var r=img.getBoundingClientRect();
    var ox=(cx-(r.left+r.width/2)), oy=(cy-(r.top+r.height/2));   // 画面上の指位置(中心からの差)
    var k=nv/scale;
    tx=(tx-ox)*k+ox; ty=(ty-oy)*k+oy;
    scale=nv;
  ...
  img.addEventListener("dblclick",function(e){ e.preventDefault(); e.stopPropagation(); zoomTo(scale>1.05?1:2.5,e.clientX,e.clientY); });
  img.addEventListener("wheel",function(e){ e.preventDefault(); zoomTo(scale*(e.deltaY<0?1.2:1/1.2),e.clientX,e.clientY,false); },{passive:false});
  ...
  img.addEventListener("touchstart",function(e){
    ...
    if(e.touches.length===2){
      ...
      pinch={d:Math.hypot(a.clientX-b2.clientX,a.clientY-b2.clientY),s:scale,
             cx:(a.clientX+b2.clientX)/2, cy:(a.clientY+b2.clientY)/2};
      drag=null;
    } else if(e.touches.length===1){
      var now=Date.now();
      if(now-lastTap<300){ e.preventDefault(); e.stopPropagation(); zoomTo(scale>1.05?1:2.5,e.touches[0].clientX,e.touches[0].clientY); lastTap=0; return; }
      lastTap=now;
  ...
  img.addEventListener("touchmove",function(e){
    ...
    if(e.touches.length===2&&pinch){
      ...
      var cx=(a.clientX+b2.clientX)/2, cy=(a.clientY+b2.clientY)/2;
      ...
      tx=drag.tx+dx; ty=drag.ty+dy; apply(false);
    }
  },{passive:false});

  img.addEventListener("touchend",function(e){
    if(pinch&&(!e.touches||e.touches.length<2)){ pinch=null; if(scale<1.05){ scale=1;tx=0;ty=0; } apply(true); }
    if(drag&&(!e.touches||e.touches.length===0)) drag=null;
  });
  var lb=document.getElementById("imageLightbox");
  if(lb&&!lb.__zr){ lb.__zr=1; lb.addEventListener("click",function(){ scale=1;tx=0;ty=0;apply(false); }); }
}
...
async function loadNotifications(kind){currentNotifKind=kind||"normal",document.querySelectorAll(".notif-kind-chip").forEach(c=>c.classList.toggle("active",c.dataset.kind===currentNotifKind));const list=document.getElementById("notificationsList");list.innerHTML=skeletonCards(4);let result;if("calls"===currentNotifKind){const __rr=await Promise.all([callApi("get_missed_calls"),callApi("get_talk_requests")]);let __m=[];if(__rr[0]&&__rr[0].ok)__m=__m.concat((__rr[0].items||[]).map(function(u){return{type:101,user_id:u.user_id,name:u.name,icon_url:u.icon_url,message:"から着信がありました",created_at:u.created_at}}));if(__rr[1]&&__rr[1].ok)__m=__m.concat((__rr[1].items||[]).map(function(u){return{type:102,user_id:u.user_id,name:u.name,icon_url:u.icon_url,message:"からトークリクエストが届きました",created_at:u.created_at}}));__m.sort(function(x,y){return String(y.created_at||"").localeCompare(String(x.created_at||""))});result={ok:!!(__rr[0]&&__rr[0].ok||__rr[1]&&__rr[1].ok),notifications:__m}}else{result=("info"===currentNotifKind?await koeLoadSystemInfo():await callApi("get_notifications",currentNotifKind))}if(!result.ok)return void(list.innerHTML=`<div class="empty-state"><div class="empty-ico">${BELL_SVG}</div>読み込みに失敗しました<br><small style="opacity:.55;">HTTP ${result.status||"?"}</small></div>`);let items=result.notifications||[];try{if(typeof isNotifTypeMuted==="function")items=items.filter(function(n){return !isNotifTypeMuted(n.type);});}catch(e){}items.length?(window.__notifItems=items,window.__notifPage=1,window.__notifDone=(result.notifications||[]).length<10,list.innerHTML=items.map((n,i)=>koeNotifHtml(n,i)).join("")+koeNotifMoreHtml()):list.innerHTML=`<div class="empty-state"><div class="empty-ico">${BELL_SVG}</div>${"important"===currentNotifKind?"重要なお知らせはありません":"info"===currentNotifKind?"運営からのお知らせはありません":"calls"===currentNotifKind?"着信・トークリクエストはありません":"通知はありません"}</div>`}function openNotifTarget(i){const n=(window.__notifItems||[])[i];if(!n)return;
 if(n&&n.info_detail){
   if((!n.body||!String(n.body).trim())&&n.url){
     try{sfx("open")}catch(e){}
     try{ if(window.AndroidApi&&window.AndroidApi.openUrl){window.AndroidApi.openUrl(n.url);return;} }catch(e){}
     try{ window.open(n.url,"_blank"); return; }catch(e){}
   }
...
document.addEventListener("keydown",e=>{if("Escape"===e.key){const lb=document.getElementById("imageLightbox");if(lb&&"flex"===lb.style.display)return void(lb.style.display="none");const open=[...document.querySelectorAll(".modal")].filter(m=>"flex"===m.style.display);open.length&&(open[open.length-1].style.display="none")}}),function(){let sy=0,tracking=!1,target=null;document.addEventListener("touchstart",e=>{if(!e.touches||!e.touches.length)return;const mc=e.target.closest(".modal-content"),modal=e.target.closest(".modal");
  ...
  }catch(e){errEl.textContent="トークンログイン処理でエラー: "+(e&&e.message?e.message:String(e)),errEl.style.display="block"}finally{btn.disabled=!1,btn.textContent="トークンでログイン"}}async function showSessionToken(){const area=document.getElementById("tokenExportArea"),result=await api().export_token();result.ok?(area.value=`token: ${result.token}\nuser_id: ${result.user_id}`,area.style.display="block",area.select(),toast("トークンを表示しました。長押しでコピーできます")):toast("トークンがありません(未ログイン)","error")}async function doSignup(){const name=document.getElementById("signupName").value.trim(),email=document.getElementById("signupEmail").value.trim(),password=document.getElementById("signupPassword").value,sex=document.getElementById("signupSex").value,birthday=document.getElementById("signupBirthday").value.trim(),errEl=document.getElementById("loginError"),btn=document.getElementById("signupBtn");errEl.style.display="none";if(errEl.textContent="",name&&email&&password&&birthday){btn.disabled=!0,btn.textContent="登録中...";try{const result=await api().signup(email,password,name,parseInt(sex,10),birthday);result.ok?(window.__koeLoginMethod="mail",enterMain(result.user_name)):(errEl.style.display="block",errEl.textContent=(result.message||"登録に失敗しました")+(result.status?` (HTTP ${result.status})`:"")+` / 応答: ${JSON.stringify(result.raw||result.body||result.error||"").slice(0,300)}`,__koeAutoDiagOnFail())}catch(e){errEl.style.display="block";errEl.textContent="登録処理でエラーが発生しました: "+(e&&e.message?e.message:String(e))}finally{btn.disabled=!1,btn.textContent="新規登録"}}else{var __miss=[];if(!name)__miss.push("ニックネーム");if(!email)__miss.push("メールアドレス");if(!password)__miss.push("パスワード");if(!birthday)__miss.push("生年月日");errEl.style.display="block";errEl.textContent="未入力の項目があります: "+__miss.join("、")}}function enterMain(userName){try{sfx("success")}catch(e){}try{setTimeout(function(){callApi("system_arrival").catch(function(){})},1500)}catch(e){}try{saveCurrentAccount()}catch(e){}document.getElementById("headerUser").textContent=userName||"",showScreen("mainScreen");
{/* 起動時シェルで既に同じタブを表示済みなら読み込みをやり直さない(同じAPIが2回走るのを防ぐ) */
 var __tab=window.__koeStartTab?window.__koeStartTab():(function(){try{return localStorage.getItem("koe_last_tab")||"timeline"}catch(e){return"timeline"}})();
 var __skip=(window.__koeShellTab===__tab); window.__koeShellTab=null;
 if(!__skip) showPage(__tab);}
startLivePulse(),callApi("get_my_profile").then(r=>{r&&r.ok&&r.profile&&(myUserId=r.profile.user_id,window.__myUserId=r.profile.user_id,setTimeout(function(){try{koeAskSaveFolder(function(){setTimeout(function(){try{koeAskActOptIn()}catch(e){}},350)})}catch(e){try{koeAskActOptIn()}catch(_){}}},900))}).catch(()=>{})}async function doLogout(){await showConfirmModal("ログアウトしますか?")&&(await (async()=>{try{await callApi("server_logout")}catch(e){}})(),await api().logout(),livePulseTimer&&(clearInterval(livePulseTimer),livePulseTimer=null),document.getElementById("loginEmail").value="",document.getElementById("loginPassword").value="",document.getElementById("loginError").textContent="",showScreen("loginScreen"))}async function downloadVoice(url){if(url&&/^https?:\/\//i.test(String(url))){if(window.AndroidApi&&window.AndroidApi.saveAudio)try{window.AndroidApi.saveAudio(url),toast("保存中…");try{haptic(10)}catch(e){}return}catch(e){}try{toast("ダウンロード中…");const res=await fetch(url),blob=await res.blob(),m=(url.split("?")[0].match(/\.[a-z0-9]{2,5}$/i)||[".m4a"])[0],a=document.createElement("a");a.href=URL.createObjectURL(blob),a.download="KoeTomo_"+Date.now()+m,document.body.appendChild(a),a.click(),setTimeout(()=>{try{URL.revokeObjectURL(a.href)}catch(e){}a.remove()},1500),toast("保存しました")}catch(e){toast("保存に失敗しました","error")}}}/* 保存形式は一度選んだら覚えておき、次からはタップだけで保存する。
   形式を変えたいときは保存ボタンを長押しする（毎回シートを挟むと手数が増えるため）。 */
function koeSyncFmtUi(){
  try{
    var a=koeDlFmtAudio(),i=koeDlFmtImg();
    var sa=document.getElementById("dlFmtAudioSel"); if(sa&&sa.value!==a) sa.value=a;
    var si=document.getElementById("dlFmtImgSel"); if(si&&si.value!==i) si.value=i;
    document.querySelectorAll(".dl-fmt-btn").forEach(function(b){b.classList.toggle("active",b.dataset.fmt===a)});
    document.querySelectorAll(".img-fmt-btn").forEach(function(b){b.classList.toggle("active",b.dataset.fmt===i)});
    var ca=document.getElementById("dlAskAudioChk"); if(ca) ca.checked=koeDlAskAudio();
    var ci=document.getElementById("dlAskImgChk"); if(ci) ci.checked=koeDlAskImg();
    var f=koeSaveFolder();
    document.querySelectorAll(".koe-folder-opt").forEach(function(b){b.classList.toggle("active",b.dataset.folder===f)});
    var fd=document.getElementById("koeFolderDesc"); if(fd) fd.textContent=KOE_FOLDER_LABEL[f]||"";
    /* シートのチェックは開くたびに外しておく（うっかり二度と出なくならないように） */
    var r1=document.getElementById("dlFmtRemember"); if(r1) r1.checked=false;
    var r2=document.getElementById("imgFmtRemember"); if(r2) r2.checked=false;
  }catch(e){}
}
window.koeSyncFmtUi=koeSyncFmtUi;
/* 「毎回きく」かどうかは音声と画像で別々。既定は毎回きく。 */
function koeDlAskAudio(){try{return localStorage.getItem("koe_dl_ask_audio")!=="0"}catch(e){return!0}}
function koeDlAskImg(){try{return localStorage.getItem("koe_dl_ask_img")!=="0"}catch(e){return!0}}
window.koeDlAskAudio=koeDlAskAudio;window.koeDlAskImg=koeDlAskImg;
/* 保存先フォルダ: app=KoeTomo専用(自動振り分け・おすすめ) / std=写真・音楽の標準 / dl=ダウンロード直下 */
function koeSaveFolder(){try{var v=localStorage.getItem("koe_save_folder");if(v==="std"||v==="dl"||v==="app")return v}catch(e){}return"std"}
function koeSetSaveFolder(mode){
  var m=(mode==="app"||mode==="dl")?mode:"std";
  try{localStorage.setItem("koe_save_folder",m)}catch(e){}
  try{ if(window.AndroidApi&&window.AndroidApi.setSaveFolder) window.AndroidApi.setSaveFolder(m); }catch(e){}
  try{koeSyncFmtUi()}catch(e){}
  return m;
}
window.koeSaveFolder=koeSaveFolder;window.koeSetSaveFolder=koeSetSaveFolder;
var KOE_FOLDER_LABEL={std:"画像は Pictures/KoeTomo、音声は Music/KoeTomo（ギャラリー・音楽アプリに出ます）",app:"Download/KoeTomo（音声は Audio、画像は Images に自動で振り分け）",dl:"ダウンロードフォルダの直下"};
function koeDlFmtAudio(){try{return localStorage.getItem("koe_dl_fmt_audio")||"original"}catch(e){return"original"}}
function koeDlFmtImg(){try{return localStorage.getItem("koe_dl_fmt_img")||"original"}catch(e){return"original"}}
window.koeDlFmtAudio=koeDlFmtAudio;window.koeDlFmtImg=koeDlFmtImg;
(function(){
  if(window.__koeDlLongPress)return;window.__koeDlLongPress=true;
  var LP=520,tm=null,longed=false,sx=0,sy=0;
  function clear(){ if(tm){clearTimeout(tm);tm=null} }
  document.addEventListener("pointerdown",function(e){
    var b=e.target.closest&&e.target.closest(".voice-dl,#lightboxSave");
    longed=false; clear(); if(!b)return;
    sx=e.clientX;sy=e.clientY;
    tm=setTimeout(function(){
      longed=true; tm=null;
      try{haptic(18)}catch(_){}
      try{
        if(b.id==="lightboxSave"){ showImgFormat(document.getElementById("lightboxImg").src) }
        else if(b.dataset.voiceUrl){ showDlFormat(b.dataset.voiceUrl) }
      }catch(_){}
    },LP);
  },true);
...
window.__pdLoadMoreComments = async function(postId){
  ...
  if((window.__pdCommentPage||1)===1 && (window.__pdComments||[]).length<10){ window.__pdCommentsDone=true; return; }
...
}async function sendPostDetailReply(){const input=document.getElementById("pdReplyInput"),text=input.value.trim();if(!text||!__pdPostId)return;input.value="";
const rbtn=document.getElementById("pdReplyBtn");rbtn&&(rbtn.disabled=!0,rbtn.classList.add("loading"));
/* 送信前に自分の返信をその場で表示（体感の即時反映）。一覧が再描画されても消えないように保持する */
var __pid=__koePdPend(text);
const result=await callApi("reply_timeline_post",__pdPostId,text);rbtn&&(rbtn.disabled=!1,rbtn.classList.remove("loading"));
if(result.ok){ try{sfx("send")}catch(e){} toast("返信しました");
  try{ var el=document.querySelector('[data-pdpend="'+__pid+'"] .pd-comment-time'); if(el) el.textContent="たった今"; }catch(e){}
  setTimeout(function(){ try{ if(__pdPostId) openPostDetail(null,__pdPostId) }catch(e){} },1200);
} else { __koePdPendDrop(__pid); input.value=text; toast(`返信失敗: ${koeErrMsg(result)}`.slice(0,120),"error"); }}
/* 送信中/送信直後の自分の返信を #pdComments に貼り付け直す（コメントの追加読み込みで消えるのを防ぐ） */
window.__pdPend=[];
function __koePdPend(text){ var id="p"+Date.now()+Math.floor(1000*Math.random());
  window.__pdPend.push({id:id,text:text,t:Date.now()}); __koePdSync();
  /* 送信後しばらくは、再描画が無くても自分で反映を確かめに行く(控えが残り続けるのを防ぐ) */
  [1800,3000,5000,8000,13000,20000,30000].forEach(function(ms){setTimeout(function(){try{__koePdSync()}catch(e){}},ms)});
  return id; }
function __koePdPendDrop(id){ try{ window.__pdPend=(window.__pdPend||[]).filter(function(x){return x.id!==id});
  var n=document.querySelector('[data-pdpend="'+id+'"]'); if(n)n.remove(); }catch(e){} }
function __koePdSync(){
  var box=document.getElementById("pdComments"); if(!box) return;
  if(!box.__pdObs){ box.__pdObs=1; try{ new MutationObserver(function(){ if(!box.__pdBusy) __koePdSync(); }).observe(box,{childList:true}); }catch(e){} }
  var list=window.__pdPend||[]; if(!list.length) return;
  box.__pdBusy=1;
  try{
    list.slice().forEach(function(it){
      ...
      var norm=function(t){return String(t==null?"":t).replace(/\s+/g,"")};
      var want=norm(it.text);
      try{
        var mine=Number(window.__myUserId||(typeof myUserId!=="undefined"?myUserId:0)||0);
        (window.__pdComments||[]).forEach(function(c){ if(serverHas)return; if(norm(c.text)===want||(mine&&Number(c.user_id)===mine&&norm(c.text).indexOf(want)>=0))serverHas=true; });
        if(!serverHas){
          ...
          for(var i=0;i<rows.length;i++){
            ...
            if(norm(tx?tx.textContent:r.innerText).indexOf(want)>=0){serverHas=true;break}
          }
        }
      }catch(e){}
      if(Date.now()-it.t>1200 && serverHas){
        window.__pdPend=window.__pdPend.filter(function(x){return x.id!==it.id});
...
try{
  ...
  if(String(uid)===String(mine)){
    window.__koeJustPosted=(window.__koeJustPosted||[]).filter(function(x){return Date.now()-x.t<120000});
    __pending=window.__koeJustPosted.filter(function(x){
      return !__posts.some(function(p){return (p.text||"")===(x.text||"")});
    });
  }
}catch(e){}
if(__pending.length){
...
window.__rtdb.streams.room_data={url:base+"/room_data.json",on:function(type,path,data){try{
  /* 購読開始時に届く「前回の残り」で誤ってダイアログを出さない(1回目は記録だけ) */
  if(window.__koeRoomDataFirst){window.__koeRoomDataFirst=false;try{window.__koeRoomDataSeen=JSON.stringify((typeof data==="string")?JSON.parse(data):data)}catch(e){}return;}
...
function startRoomCommentPolling(){try{if(!window.__koeIconBase)callApi("get_icon_base").then(function(r){if(r&&r.ok&&r.base)window.__koeIconBase=r.base}).catch(function(){})}catch(e){}stopRoomCommentPolling();window.__chatNotified={};window.__chatPending=[];window.__chatMine=[];window.__chatStore=[];window.__chatSeq=0;window.__chatSys=[];window.__chatNotifiedInit=false;window.__chatUnread=0;window.__callLogLines=[];window.__sysSeen=0;
try{var __sl=document.getElementById("callSysLog");if(__sl){__sl.innerHTML="";__sl.__sig=""}}catch(e){}
try{if(typeof koeSwitchCallTab==="function")koeSwitchCallTab("chat")}catch(e){}try{var __lg=document.getElementById("callChatLog");if(__lg){__lg.innerHTML="";__lg.__sig=""}}catch(e){}try{var tg=document.getElementById("callChatToggle"),bd=tg&&tg.querySelector(".callv2-badge");if(bd)bd.style.display="none"}catch(e){}try{koeRtdbStart(currentRoomId)}catch(e){}roomCommentTimer=setInterval(function(){reloadRoomComments()},1500),reloadRoomComments()}function stopRoomCommentPolling(){try{koeRtdbStop()}catch(e){}roomCommentTimer&&(clearInterval(roomCommentTimer),roomCommentTimer=null)}
...
function koeActOptInMap(){try{return JSON.parse(localStorage.getItem("koe_act_optin")||"null")||{}}catch(e){return{}}}
function koeActUid(){try{return String((typeof myUserId!=="undefined"&&myUserId)||window.__myUserId||"")}catch(e){return""}}
function koeActOptIn(){var u=koeActUid();if(!u)return null;var m=koeActOptInMap();return u in m?!!m[u]:null}
function koeActSetOptIn(on){var u=koeActUid();if(!u)return;var m=koeActOptInMap();m[u]=!!on;try{localStorage.setItem("koe_act_optin",JSON.stringify(m))}catch(e){}
 if(!on){try{localStorage.removeItem("koe_act_hours");localStorage.removeItem("koe_act_seen")}catch(e){}}
 try{var c=document.getElementById("koeActOnChk");if(c)c.checked=!!on}catch(e){}
 try{if(typeof koeActivityRender==="function")koeActivityRender()}catch(e){}}
/* インストール後の初回に一度だけ、保存先フォルダを選んでもらう。
   おすすめ（KoeTomo 専用フォルダ・音声/画像を自動で振り分け）を最初から選んだ状態にする。 */
function koeAskSaveFolder(after){
  var done=false; try{ done=localStorage.getItem("koe_save_folder_asked")==="1" }catch(e){}
  if(done||document.getElementById("koeFolderAskModal")){ if(after)try{after()}catch(e){} return }
  var d=document.createElement("div"); d.id="koeFolderAskModal"; d.className="modal"; d.style.display="flex";
  d.innerHTML='<div class="modal-content"><div class="modal-header"><h3>保存先を決めてください</h3></div>'+
  ...
  d.querySelectorAll(".koe-fa-opt").forEach(function(b){
    b.addEventListener("click",function(){
      ...
      d.querySelectorAll(".koe-fa-opt").forEach(function(x){x.classList.toggle("active",x===b)});
      try{sfx("select");haptic(8)}catch(e){}
    });
  });
  d.querySelector("#koeFaOk").addEventListener("click",function(){
    koeSetSaveFolder(pick);
    try{localStorage.setItem("koe_save_folder_asked","1")}catch(e){}
    try{d.remove()}catch(e){}
    try{toast("保存先を設定しました")}catch(e){}
...
function koeAskActOptIn(){
  ...
  var d=document.createElement("div");d.id="koeActAskModal";d.className="modal";d.style.display="flex";
  d.innerHTML='<div class="modal-content small"><div class="modal-header"><h3>活動時間帯の集計</h3></div>'+
...
function koeSpamScore(u){try{if(!u)return{score:0,reasons:[]};var sc=0,rs=[];
  ...
  var hasr=function(k){return ru&&ru[k]!==undefined&&ru[k]!==null};
  var a1=/^[A-Za-z0-9]{16}\.(png|jpe?g|webp)$/i.test(fn);
  var a2=has("follower_count")&&has("followee_count")&&has("friend_count")&&has("liked_count")
        &&Number(u.follower_count)===0&&Number(u.followee_count)===0&&Number(u.friend_count)===0&&Number(u.liked_count)===0;
  var a3=("comment" in u)&&!String(u.comment||"").trim();
  var av=hasr("age_verification_status")?Number(ru.age_verification_status):(has("age_verification")?Number(u.age_verification):NaN);
  var a4=(av===0);
  var hard=a1&&a2&&a3&&a4;
  if(!hard) return {score:0,level:"",reasons:[],hard:false};
  rs.push("量産型アイコン名");rs.push("交流0・自己紹介なし・年齢確認なし");
  var nm=String(u.name||"");if(/^[^\s]{1,20}\d{3}$/.test(nm)&&!/^\d+$/.test(nm)){sc+=3;rs.push("名前が単語+3桁数字")}
  if(ru&&ru.settings&&ru.settings.random_match_enabled===true){sc+=1.5;rs.push("ランダムマッチON+交流0")}
  else if(ru&&ru.random_match_enabled===true){sc+=1.5;rs.push("ランダムマッチON+交流0")}
  var ls=String((ru&&ru.login_status_with_unit)||u.login_status||"");
  if(/1時間以内|分以内|オンライン/.test(ls)){sc+=0.5;rs.push("直近ログイン")}
  var lv0=(sc>=6)?"high":(sc>=3)?"mid":"";
  return{score:sc,level:lv0,reasons:rs,hard:true}}catch(e){return{score:0,reasons:[]}}}
...
function koeBotAutoOn(){try{var v=localStorage.getItem("koe_bot_auto");return v===null?true:v==="1"}catch(e){return true}}
function koeBotHint(u){try{if(!u)return false;
  var icon=String(u.icon_url||u.profile_picture_file_path||"");var fn=icon.split("?")[0].split("/").pop()||"";
  if(!/^[A-Za-z0-9]{16}\.(png|jpe?g|webp)$/i.test(fn))return false;
  var nm=String(u.name||"");
  if(/^[^\s]{1,20}\d{3}$/.test(nm)&&!/^\d+$/.test(nm))return true;
  var z=function(k){return u[k]!==undefined&&u[k]!==null&&Number(u[k])===0};
  return z("follower_count")&&z("followee_count");
}catch(e){return false}}
var __koeBotQ=[],__koeBotSeen={},__koeBotTimer=null;
function koeBotQueue(uid){try{
  if(!uid||!koeBotAutoOn())return; uid=String(uid);
  if(__koeBotSeen[uid])return; __koeBotSeen[uid]=1;
  var s=[]; try{s=JSON.parse(localStorage.getItem("koe_bot_scanned")||"[]")}catch(e){s=[]}
  if(s.indexOf(uid)>=0)return;
  if(__koeBotQ.length>=40)return;
  __koeBotQ.push(uid); koeBotPump();
}catch(e){}}
function koeBotPump(){ if(__koeBotTimer)return;
  __koeBotTimer=setInterval(function(){
    if(!__koeBotQ.length){ try{clearInterval(__koeBotTimer)}catch(e){} __koeBotTimer=null; return }
    if(document.hidden||!koeBotAutoOn())return;
    var uid=__koeBotQ.shift(); if(!uid)return;
    try{ var s=[]; try{s=JSON.parse(localStorage.getItem("koe_bot_scanned")||"[]")}catch(e){s=[]}
      s.unshift(uid); if(s.length>300)s=s.slice(0,300); localStorage.setItem("koe_bot_scanned",JSON.stringify(s)); }catch(e){}
    Promise.resolve(callApi("moderation_auto_spam",KOE_BL_URL,uid)).then(function(r){
      try{ if(r&&r.applied){ toast("業者アカウントを自動でBANリストに申請しました（ブロックはしていません）"); } }catch(e){}
    }).catch(function(){});
  },25000);
}
try{ document.addEventListener("visibilitychange",function(){ if(!document.hidden&&__koeBotQ.length) koeBotPump(); }); }catch(e){}
function koeSpamHide(){try{return localStorage.getItem("koe_spam_hide")==="1"}catch(e){return false}}
function koeSpamTag(u,compact){try{if(!koeSpamEnabled())return"";
  var r=koeSpamScore(u);
  if(!r.level&&koeBotHint(u)){ r={level:"mid",reasons:["量産型アイコン名","名前・交流の傾向"],score:0}; }
  if(!r.level)return"";
  try{ if(u&&u.user_id) koeBotQueue(u.user_id) }catch(e){}
...
function koeSpamQuota(add){ /* 申請の間隔と件数を端末側で制限（サーバーの「短時間に多すぎ」対策） */
  try{
    var now=Date.now(), h=[];
    try{ h=JSON.parse(localStorage.getItem("koe_spam_reports")||"[]") }catch(e){ h=[] }
    h=h.filter(function(x){return now-x<86400000});
    var last=h.length?Math.max.apply(null,h):0;
    var inHour=h.filter(function(x){return now-x<3600000}).length;
    if(add){ h.push(now); try{localStorage.setItem("koe_spam_reports",JSON.stringify(h))}catch(e){} return {ok:true} }
    if(now-last<60000) return {ok:false,msg:"連続で申請はできません。"+Math.ceil((60000-(now-last))/1000)+"秒後にお試しください"};
...
async function koeSpamReport(uid,reasonText,nm){try{var url=(typeof BANLIST_FIXED_URL!=="undefined"&&BANLIST_FIXED_URL)||"https://redredfast.com";
  var q=koeSpamQuota(false); if(!q.ok){ toast(q.msg,"error"); return false; }
  /* 理由・証拠はアプリ(ネイティブ側)がその場で API から取得して判定する。画面上の文字列は送らない(偽造防止) */
  var r=await callApi("moderation_report_spam",url,String(uid));
  if(r&&r.ok){koeSpamQuota(true);toast(r.duplicate?"この相手は既に申請されています。ご協力ありがとうございます":"申請しました。コミュニティへの貢献にありがとうございます！承認されると共有BANリストに反映されます");try{sfx("success")}catch(e){}return true}
  if(r&&r.error==="not_spam_like"){toast(r.message||"業者判定の条件を満たしていません","error");return false}
  if(r&&r.error==="rate_limited"){koeSpamQuota(true);toast("サーバー側で受付が制限されています。しばらく時間をおいてからお試しください","error");return false}
...
function koeSysRender(list){
  try{
    var box=document.getElementById("callSysLog"); if(!box)return;
    var Y=list||window.__chatSys||[];
    var extra=window.__callLogLines||[];
    var rows=[];
    Y.forEach(function(y){rows.push({t:y.addedAt||0,h:'<div class="koe-chat-sys">'+escapeHtml(y.text)+'</div>'})});
    extra.forEach(function(l){rows.push({t:l.t||0,h:'<div class="koe-chat-sys koe-chat-sys-dim">'+escapeHtml(l.text)+'</div>'})});
    rows.sort(function(a,b){return a.t-b.t});
    var sig=rows.length+"#"+(rows.length?rows[rows.length-1].h:"");
    if(sig===box.__sig)return; box.__sig=sig;
    var atBottom=box.scrollHeight-box.scrollTop-box.clientHeight<50;
    box.innerHTML=rows.length?rows.map(function(r){return r.h}).join(""):'<div class="empty-msg" style="padding:8px 0;">まだ何もありません</div>';
    if(atBottom)box.scrollTop=box.scrollHeight;
    var c=document.getElementById("callSysCount");
    if(c){
      var unseen=rows.length-(window.__sysSeen||0);
      if(unseen>0&&document.querySelector('.callv2-tab[data-ctab="log"]:not(.active)')){c.textContent=unseen>99?"99+":String(unseen);c.style.display=""}
...
function koeSwitchCallTab(which){
  try{
    // チャットとログは1つにまとめたので、タブでの切り替えはしない(両方いつも表示)。
    var chat=document.getElementById("callChatLog"), sys=document.getElementById("callSysLog");
    var inp=document.querySelector("#callChatPanel .callv2-chatinput");
    var note=document.querySelector("#callChatPanel .callv2-note");
    var tabs=document.querySelector("#callChatPanel .callv2-tabs"); if(tabs)tabs.style.display="none";
    if(chat)chat.style.display="";
    if(sys)sys.style.display="";
    if(inp)inp.style.display="";
    if(note)note.style.display="";
    // 未読カウントは常に消しておく(1つにまとめたので)
    window.__sysSeen=((window.__chatSys||[]).length)+((window.__callLogLines||[]).length);
    var cc=document.getElementById("callSysCount"); if(cc)cc.style.display="none";
  }catch(e){}
}
...
(function(){
  var PAIRS=[["callAutoRaiseChk","autoRaiseHandChk"],["callAutoApproveChk","autoApproveChk"],["callAutoRejectChk","autoRejectChk"]];
  function sync(){
    PAIRS.forEach(function(p){
      ...
      if(!a.__koeB){
        ...
        a.addEventListener("change",function(){
          b.checked=a.checked;
          /* 「許可」と「拒否」は同時に選べない */
          if(a.id==="callAutoApproveChk"&&a.checked){var r=document.getElementById("callAutoRejectChk"),rb=document.getElementById("autoRejectChk");if(r)r.checked=false;if(rb)rb.checked=false}
          if(a.id==="callAutoRejectChk"&&a.checked){var q=document.getElementById("callAutoApproveChk"),qb=document.getElementById("autoApproveChk");if(q)q.checked=false;if(qb)qb.checked=false}
          try{b.dispatchEvent(new Event("change"))}catch(e){}
          try{var sv=document.getElementById("saveModerationBtn"); if(sv)sv.click()}catch(e){}
          try{toast(a.checked?"オンにしました":"オフにしました")}catch(e){}
        });
      }
    });
  }
  ...
  document.addEventListener("click",function(ev){
    try{ if(ev.target&&ev.target.closest&&ev.target.closest("#callSettingsToggle")) setTimeout(sync,60); }catch(e){}
  });
})();
function koeChatRender(){
  ...
  var sig=S.map(function(c){return c.k+(c.explicit?"e":"")+(c.penalty?"p":"")}).join(",")+"#"+P.map(function(p){return p.id+(p.failed?"f":"")}).join(",");
...
if(!cmtOn){try{const rid=result.room_id||currentRoomId;if(rid){const sw=await callApi("room_switch_comment_enabled",String(rid),false);if(!sw||sw.ok===false)toast(koeErrMsg(sw),"error")}}catch(e){}}
return}
...
(function(){
  function bind(){
    ...
    if(db&&!db.__koeBound){db.__koeBound=true;db.addEventListener("click",async function(){var ids=Array.from(window.__koeChatSelected);if(!ids.length)return;if(!await showConfirmModal(ids.length+"件のチャットを削除します。取り消せませんがよろしいですか?"))return;db.disabled=true;var r=await callApi("bulk_delete_chats",ids.join(","));toast(r&&r.ok?("削除しました"+(r.deleted?"("+r.deleted+"件)":"")):((r&&r.message)?String(r.message):"削除に失敗しました"),r&&r.ok?undefined:"error");window.__koeChatSelected.clear();window.__koeChatEditMode=false;var eb2=document.getElementById("chatEditBtn");if(eb2)eb2.textContent="編集";loadChats();});}
  }
  if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",bind)}else{bind()}
  setTimeout(bind,1200);
  document.addEventListener("click",function(){setTimeout(bind,300);},true);
})();
async function openChat(chatId,targetId,name,iconUrl){currentChat={chatId:chatId,targetId:targetId,name:name,icon:iconUrl};try{var head=document.getElementById("chatModalPartner");if(head){head.innerHTML=avatarHtml(name,iconUrl)+'<span id="chatModalTitle"></span>';head.onclick=function(){if(currentChat&&currentChat.targetId){window.__koeProfileOpenedFromChat=true;var cm=document.getElementById("chatModal");if(cm)cm.style.display="none";viewProfile(currentChat.targetId);}};}var t=document.getElementById("chatModalTitle");if(t)t.textContent=name;}catch(e){document.getElementById("chatModalTitle").textContent=name;}document.getElementById("chatModal").style.display="flex",await reloadMessages(),startChatPolling();try{koePusherSubscribe(currentChat&&currentChat.chatId)}catch(e){}}let chatPollTimer=null;function startChatPolling(){stopChatPolling(),chatPollTimer=setInterval(()=>{const m=document.getElementById("chatModal");currentChat&&m&&"none"!==m.style.display?reloadMessages(!0):stopChatPolling()},1800)}function stopChatPolling(){chatPollTimer&&(clearInterval(chatPollTimer),chatPollTimer=null)}
// 送信直後に画面へ即反映するための保留エントリ
function koeChatPending(obj){
  try{ if(!currentChat) return; currentChat.pending=currentChat.pending||[];
    obj.t=Date.now(); currentChat.pending.push(obj);
    var box=document.getElementById("chatMessages");
    if(box){ var el=document.createElement("div"); el.className="msg-row mine";
      el.innerHTML='<div class="msg-bubble mine sending">'+
        (obj.kind==="img"?'<img class="chat-msg-img" src="'+escAttr(obj.data)+'" style="cursor:zoom-in" onclick="openLightbox(this.src)">':"")+
        (obj.kind==="voice"?'<audio class="chat-msg-voice" controls src="'+escAttr(obj.data)+'" style="max-width:220px;width:100%"></audio>':"")+
        (obj.text?escapeHtml(obj.text):"")+'</div><div class="msg-meta">送信中…</div>';
...
var __pusherWS=null,__pusherChan=null,__pusherCfg=null,__pusherRetry=0;
async function koePusherEnsure(){
  if(__pusherCfg) return __pusherCfg;
  try{ var r=await callApi("get_pusher_config"); if(r&&r.ok&&r.key){ __pusherCfg={key:r.key,cluster:r.cluster||"ap3"}; } }catch(e){}
  return __pusherCfg;
}
async function koePusherUser(){
  /* 自分のユーザーIDのチャンネルも購読しておく（届けば通知が即時になる） */
  try{
    var cfg=await koePusherEnsure(); if(!cfg) return;
    var uid=(typeof myUserId!=="undefined"&&myUserId)||currentAccountId(); if(!uid) return;
    if(window.__pusherUserWS&&window.__pusherUserWS.readyState<=1) return;
    var ws=new WebSocket("wss://ws-"+cfg.cluster+".pusher.com/app/"+cfg.key+"?protocol=7&client=koetomoplus&version=1.0");
    ...
    ws.onmessage=function(ev){
      try{
        ...
        if(d.event&&d.event.indexOf("pusher")!==0){
          try{ callApi("log","[PUSHER] user event "+d.event+" ch="+(d.channel||"")) }catch(e){}
          try{ if(typeof loadNotifications==="function"){ var act=document.querySelector(".page.active"); if(act&&act.id==="page-notifications") loadNotifications(typeof currentNotifKind!=="undefined"?currentNotifKind:"normal"); } }catch(e){}
          try{ callApi("get_unread_notif_count").then(function(r){ var c=r&&(r.count!==undefined?r.count:r.unread_count); var b=document.getElementById("notifBadge"); if(b&&typeof c==="number"){ if(c>0){b.textContent=c>99?"99+":String(c);b.style.display="block"; sfx("notify")} } }) }catch(e){}
        }
      }catch(e){}
    };
    ws.onclose=function(){ window.__pusherUserWS=null; setTimeout(koePusherUser, 15000); };
    ws.onerror=function(){ try{ws.close()}catch(e){} };
  }catch(e){}
}
try{ setTimeout(koePusherUser, 3000); }catch(e){}
async function koePusherSubscribe(chatId){
  try{
    ...
    koePusherClose();
    __pusherChan=chatId;
    var url="wss://ws-"+cfg.cluster+".pusher.com/app/"+cfg.key+"?protocol=7&client=koetomoplus&version=1.0";
    var ws=new WebSocket(url); __pusherWS=ws;
    ws.onopen=function(){ __pusherRetry=0; };
    ws.onmessage=function(ev){
      try{
        var d=JSON.parse(ev.data||"{}");
        if(d.event==="pusher:connection_established"){
          ws.send(JSON.stringify({event:"pusher:subscribe",data:{channel:__pusherChan}}));
          try{ callApi("log","[PUSHER] subscribe "+__pusherChan) }catch(e){}
          return;
        }
        if(d.event==="pusher:ping"){ ws.send(JSON.stringify({event:"pusher:pong",data:{}})); return; }
        if(d.event==="message"){
          ...
          try{ if(typeof sfx==="function") sfx("msg_in") }catch(e){}
        }
      }catch(e){}
    };
    ws.onclose=function(){
      if(__pusherChan&&currentChat&&__pusherRetry<5){
        __pusherRetry++;
        setTimeout(function(){ if(currentChat) koePusherSubscribe(__pusherChan) }, 1000*__pusherRetry);
      }
    };
    ws.onerror=function(){ try{ws.close()}catch(e){} };
  }catch(e){}
...
function koePusherClose(){
  try{ if(__pusherWS){ __pusherWS.onclose=null; __pusherWS.close(); } }catch(e){}
  __pusherWS=null;
}
...
async function koeDeleteMyMessage(ev,mid){
  ...
  var r;try{r=await callApi("delete_chat_message",String(mid),
      String((currentChat&&currentChat.chatId)||""),String((currentChat&&currentChat.targetId)||""))}catch(e){r=null}
  if(r&&r.ok){
    try{
      if(currentChat&&currentChat.seen)delete currentChat.seen[String(mid)];
      if(currentChat)currentChat.__deleted=(currentChat.__deleted||{}),currentChat.__deleted[String(mid)]=1;
      var row=document.querySelector('#chatMessages [data-mid="'+String(mid)+'"]');
      if(row&&row.parentNode)row.parentNode.removeChild(row);
      var box=document.getElementById("chatMessages"); if(box){box.__keys="";box.__sigs="";box.__rowCount=0}
    }catch(e){}
    try{toast("削除しました");sfx("close")}catch(e){}
    try{reloadMessages(true)}catch(e){}
  }else{
    if(btn)btn.disabled=false;
    try{toast((r&&r.message)?String(r.message):("削除できませんでした"+(r&&r.status?" (HTTP "+r.status+")":"")),"error")}catch(e){}
  }
}
async function reloadMessages(isPoll){if(!currentChat)return;const box=document.getElementById("chatMessages");isPoll||(box.innerHTML='<div class="empty-msg">読み込み中...</div>');const __cid=currentChat.chatId;const result=await callApi("get_messages",currentChat.chatId,currentChat.targetId);if(!currentChat||currentChat.chatId!==__cid)return;if(!result.ok)return void(isPoll||(box.innerHTML='<div class="empty-msg">読み込み失敗</div>'));if(!result.messages.length)return void(isPoll||(box.innerHTML=`<div class="empty-msg">メッセージがありません<br><small style="opacity:.55;word-break:break-all;">${escapeHtml(result.raw||"")}</small></div>`));if(!currentChat.seen)currentChat.seen={};var __seen=currentChat.seen,__curSet={},__curIds=[];result.messages.forEach(function(m){if(m.id!=null){var k=String(m.id);
if(currentChat.__deleted&&currentChat.__deleted[k])return;__curSet[k]=1;__curIds.push(Number(m.id));var e=__seen[k];if(e){e.text=m.text;e.is_read=m.is_read;e.sent_at=m.sent_at;e.img=m.image_url;e.voice=m.voice_url||"";e.play_time=m.play_time||0;e.deleted=false}else{__seen[k]={id:m.id,text:m.text,is_read:m.is_read,sent_at:m.sent_at,img:m.image_url,voice:m.voice_url||"",play_time:m.play_time||0,mine:m.user_id===result.my_user_id,ord:currentChat.__ord=(currentChat.__ord||0)+1}}}});var __minId=__curIds.length?Math.min.apply(null,__curIds):0,__newlyDeleted=0;Object.keys(__seen).forEach(function(k){var e=__seen[k];if(!e.deleted&&!__curSet[k]&&Number(k)>=__minId){e.deleted=true;__newlyDeleted++}});if(isPoll&&__newlyDeleted>0){try{sfx("notify")}catch(e){}toast(" 相手がメッセージを削除しました")}var __list=Object.keys(__seen).map(function(k){return __seen[k]}).sort(function(a,b){var d=(Number(a.id)||0)-(Number(b.id)||0);return d!==0?d:(a.ord||0)-(b.ord||0)});
/* 送信直後の「反映待ち」を先に出す（サーバー反映後は自動で消える） */
try{
  var pend=(currentChat.pending||[]).filter(function(x){
    if(x.text) return !__list.some(function(m){return m.mine&&m.text===x.text});
    if(x.kind) return (Date.now()-x.t)<20000 && !__list.some(function(m){return m.mine&&((x.kind==="img"&&m.img)||(x.kind==="voice"&&m.voice))&&(Date.now()-x.t)<20000&&m.id&&x.after&&Number(m.id)>Number(x.after)});
    return false;
  });
  currentChat.pending=pend;
  pend.forEach(function(x){ __list.push({id:9e15+(x.t%1e6),mine:true,text:x.text||"",img:x.kind==="img"?x.data:"",voice:x.kind==="voice"?x.data:"",play_time:x.play_time||0,sending:true}) });
...
const __rows=__list.map(m=>{
  ...
  const sig=[mine?1:0,m.deleted?1:0,m.sending?1:0,m.text||"",m.img||"",m.voice||"",m.play_time||0].join("\u0001");
  const timeHtml=m.sent_at?`<span class="msg-time" title="${escapeHtml(m.sent_at)}">${relTime(m.sent_at)}</span>`:"";
  ...
  var delBtn=(mine&&!m.deleted&&!m.sending&&String(m.id).length<15)
      ? '<button type="button" class="msg-del" onclick="koeDeleteMyMessage(event,&quot;'+String(m.id).replace(/[^0-9]/g,"")+'&quot;)">削除</button>' : "";
  return {key:key,sig:sig+"|"+(delBtn?1:0),read:readHtml,time:timeHtml,del:delBtn,
    html:`<div class="msg-row ${mine?"mine":"theirs"}" data-mid="${escapeHtml(key)}">\n      ${body}\n      <div class="msg-meta">${delBtn}${readHtml}${timeHtml}</div>\n    </div>`};
});
const __keys=__rows.map(r=>r.key).join(","),__sigs=__rows.map(r=>r.sig).join("\u0002");
...
async function koeLoadFriendCount(){
  try{
    if(window.__koeFriendCount!=null){koeSyncFriendCount();return}
    var r=await callApi("get_friends_list","1");
    if(r&&r.ok){window.__koeFriendCount=(r.users||[]).length;koeSyncFriendCount()}
  }catch(e){}
}
async function openFollowList(userId,kind){
  window.__koeLastFollowList={userId:userId,kind:kind};
  const modal=document.getElementById("followListModal"),body=document.getElementById("followListBody");
  const isFriends=("friends"===kind||"mutual"===kind);
  window.__koeFlKind=isFriends?"friends":(("followers"===kind)?"followers":"followees");
  try{var __me=(typeof myUserId!=="undefined"&&myUserId)||window.__myUserId||"";window.__koeFlOwn=(String(userId)===String(__me))}catch(e){window.__koeFlOwn=false}
  const isFollowers=("followers"===kind);
  document.getElementById("followListTitle").textContent=isFriends?"友達":isFollowers?"フォロワー":"フォロー中";
  body.innerHTML=skeletonCards(4); modal.style.display="flex";
  /* 友達は koetomo 本体の友達一覧(relation/friends)。相互フォローとは別物なので専用APIを使う。 */
  const method=isFriends?"get_friends_list":(isFollowers?"get_followers":"get_followees");
  let result;try{result=isFriends?await callApi(method,"1"):await callApi(method,userId,1)}catch(e){result=null}
  if(!result||!result.ok){body.innerHTML='<div class="empty-msg">取得できませんでした</div>';return}
  /* 友達(自分の一覧)は公開設定と関係ないので、非公開扱いにしない */
  if(!isFriends&&!(result.users||[]).length&&window.__koeLastProfile&&String(window.__koeLastProfile.user_id)===String(userId)
     &&!koeListPublic(window.__koeLastProfile,isFollowers?"follower":"followee")){
    body.innerHTML='<div class="empty-msg">この人は一覧を非公開にしています</div>';return}
  let us=(result.users||[]).slice();
  if(!us.length&&!isFriends){body.innerHTML='<div class="empty-msg">'+(isFollowers?"フォロワーがいません":"フォローしている人がいません")+'</div>';return}
  try{us.sort(function(a,b){var x=koeParseLoginAgo(a.login_status),y=koeParseLoginAgo(b.login_status);
    if(x===null&&y===null)return 0;if(x===null)return 1;if(y===null)return -1;return x-y})}catch(e){}
  try{var me=(typeof myUserId!=="undefined"&&myUserId)||window.__myUserId||"";
      if(String(userId)===String(me)&&!isFollowers)koeActivityRecord(us)}catch(e){}
  try{
    var meId=(typeof myUserId!=="undefined"&&myUserId)||window.__myUserId||"";
    if(String(userId)===String(meId)){
      if(!isFriends)us.forEach(function(u){
        if(!isFollowers){ if(!u.requested) u.is_following=true; }
        else { u.is_followed=true; }
      });
    }
  }catch(e){}
  if(isFriends){try{window.__koeFriendCount=us.length;koeSyncFriendCount()}catch(e){}}
  window.__koeFollowListUsers=us;
  if(isFriends){
    /* 片方が0人ならもう片方を開く(「友達がいません」だけ見えて終わるのを防ぐ) */
    var nM=us.filter(koeIsMutual).length,nF=us.filter(koeIsFriend).length;
  ...
  body.innerHTML=(isFriends?koeFlTabsHtml(us):"")
    ...
    +'<input id="followListSearch" type="text" placeholder="'+(isFriends?"友達":isFollowers?"フォロワー":"フォロー中")+'を検索" autocomplete="off"></div>'
    +'<div id="followListRows"></div>';
  koeRenderFollowRows("");
  try{
    var si=document.getElementById("followListSearch");
    if(si)si.addEventListener("input",function(){koeRenderFollowRows(this.value||"")});
  }catch(e){}
  if(isFriends)koeLoadFriendRequests();
}

/* 届いている友達申請。友達一覧の一番上にまとめて出し、その場で承認/拒否できる。 */
async function koeLoadFriendRequests(){
  var box=document.getElementById("followListBody"); if(!box)return;
  var r;try{r=await callApi("get_friend_requests_in")}catch(e){r=null}
  var us=(r&&r.ok&&r.users)||[];
  var old=document.getElementById("flReqBox"); if(old)old.remove();
  if(!us.length)return;
  var html='<div id="flReqBox" class="fl-reqbox"><div class="fl-reqhead">届いている友達申請 '+us.length+'件</div>'
    +us.map(function(u){
      var uid=Number(u.user_id)||0;
      return '<div class="fl-row" data-requid="'+uid+'">'
        +'<span class="fl-tap" onclick="closeFollowList(); viewProfile('+uid+')">'+avatarHtml(u.name,u.icon_url)+'</span>'
        +'<div class="fl-body" onclick="closeFollowList(); viewProfile('+uid+')"><div class="fl-name">'+escapeHtml(u.name||("user "+uid))+'</div></div>'
        +'<button type="button" class="fl-pill is-on" onclick="koeAnswerFriendRequest(event,'+uid+',1)">承認</button>'
        +'<button type="button" class="fl-pill is-off" onclick="koeAnswerFriendRequest(event,'+uid+',0)">拒否</button>'
        +'</div>';
    }).join("")+'</div>';
...
async function koeAnswerFriendRequest(ev,uid,ok){
  try{ev.stopPropagation();ev.preventDefault()}catch(e){}
  var btn=ev.currentTarget||ev.target; if(btn)btn.disabled=true;
  var row=document.querySelector('.fl-row[data-requid="'+(Number(uid)||0)+'"]');
  var r;try{r=await callApi(ok?"send_friend_request":"deny_friend_request",uid)}catch(e){r=null}
  if(btn)btn.disabled=false;
  if(r&&r.ok){
    if(row&&row.parentNode)row.parentNode.removeChild(row);
    try{var hd=document.querySelector("#flReqBox .fl-reqhead");
        var left=document.querySelectorAll("#flReqBox .fl-row").length;
        if(!left){var b=document.getElementById("flReqBox");if(b)b.remove()}
        else if(hd)hd.textContent="届いている友達申請 "+left+"件"}catch(e){}
    try{window.__koeFriendCount=null;koeLoadFriendCount()}catch(e){}
    try{toast(ok?"友達になりました":"申請を断りました")}catch(e){}
    ...
    if(ok){try{
      ...
      if(!us.some(function(u){return String(u.user_id)===String(uid)})){
        us.unshift({user_id:uid,name:nm,icon_url:"",is_friend:true,is_following:false,is_followed:false});
        window.__koeFollowListUsers=us;
      }else{
        us.forEach(function(u){if(String(u.user_id)===String(uid)){u.is_friend=true;u.friend_requested=false}});
      }
      koeRenderFollowRows((document.getElementById("followListSearch")||{}).value||"");
    }catch(e){}}
  }else{
    try{toast(koeErrMsg(r||{})||"できませんでした","error")}catch(e){}
  }
}
...
function koeFollowRowHtml(u){
  var uid=Number(u.user_id)||0;
  var st=koeOnlineState(u.login_status);
  ...
  var rel=u.requested?"リクエスト中"
         :(isFriend&&isMutual)?"友達・相互"
         :isFriend?"友達"
         :isMutual?"相互"
         :u.is_followed?"自分のフォロワー"
         :u.is_following?"フォロー中"
         :"";
  ...
  var sub=(rel?'<span class="fl-rel">'+rel+'</span>':"")+(rel&&online?'<span class="fl-dot">・</span>':"")+online;
  return '<div class="fl-row" data-uid="'+uid+'">'
    +'<span class="fl-tap" onclick="closeFollowList(); viewProfile('+uid+')">'+avatarHtml(u.name,u.icon_url)+'</span>'
    +'<div class="fl-body" onclick="closeFollowList(); viewProfile('+uid+')">'
      +'<div class="fl-name">'+escapeHtml(u.name||("user "+uid))+koeSpamTag(u)+'</div>'
      +(sub?'<div class="fl-sub">'+sub+'</div>':'')
    +'</div>'
    +((window.__koeFlKind==="followers"&&window.__koeFlOwn)
        ? '<button type="button" class="fl-x" title="フォロワーから外す" onclick="koeRemoveFollower(event,'+uid+')">×</button>'
        : "")
    +(window.__koeFlKind==="friends"
        /* 友達も相互も同じ一覧。申請は相手のプロフィールから行うので、ここはプロフィールへ行くだけ。 */
        ? '<button type="button" class="fl-pill is-go" onclick="closeFollowList(); viewProfile('+uid+')">プロフィール見に行く</button>'
...
function koeFriendPillHtml(uid,state){
  ...
  return '<button type="button" class="'+cls+'" data-fuid="'+uid+'" data-fstate="'+state+'" onclick="koeFriendPillTap(event,'+uid+')">'+label+'</button>';
}
async function koeFriendPillTap(ev,uid){
  try{ev.stopPropagation();ev.preventDefault()}catch(e){}
  var btn=ev.currentTarget||ev.target; if(!btn||btn.disabled)return;
  ...
  var r;try{r=await callApi(method,uid)}catch(e){r=null}
  btn.disabled=false;
  if(r&&r.ok){
    try{(window.__koeFollowListUsers||[]).forEach(function(u){
      if(String(u.user_id)===String(uid)){
        if(next==="friend"){u.is_friend=true;u.friend_requested=false}
        else u.friend_requested=(next==="sent")
      }})}catch(e){}
    try{window.__koeFriendCount=null;koeLoadFriendCount()}catch(e){}
    try{toast(next==="none"?"友達申請を取り消しました":next==="friend"?"友達になりました":"友達申請を送りました")}catch(e){}
    if(next==="friend"){try{koeRenderFollowRows((document.getElementById("followListSearch")||{}).value||"")}catch(e){}}
  }else{
    koeSetFriendUi(uid,was);
    try{toast(koeErrMsg(r||{})||"友達申請できませんでした","error")}catch(e){}
  }
}
function koeSetFriendUi(uid,state){
  try{
    document.querySelectorAll('.fl-pill[data-fuid="'+(Number(uid)||0)+'"]').forEach(function(b){
      ...
      b.className=f.className; b.innerHTML=f.innerHTML; b.setAttribute("data-fstate",state);
    });
  }catch(e){}
  try{
...
async function koeRemoveFollower(ev,uid){
  ...
  btn.disabled=true;
  var r;try{r=await callApi("remove_follower",uid)}catch(e){r=null}
  btn.disabled=false;
  if(r&&r.ok){
    try{window.__koeFollowListUsers=(window.__koeFollowListUsers||[]).filter(function(u){return String(u.user_id)!==String(uid)})}catch(e){}
    koeRenderFollowRows((document.getElementById("followListSearch")||{}).value||"");
    try{toast("フォロワーから外しました")}catch(e){}
  }else{
    try{toast(koeErrMsg(r||{})||"外せませんでした","error")}catch(e){}
  }
}
/* 運営からのお知らせ。公式のお知らせ画面と同じ POST api/system/info。 */
async function koeLoadSystemInfo(){
  var r;try{r=await callApi("get_system_info","0","1")}catch(e){r=null}
  if(!r||!r.ok)return {ok:false,status:(r&&r.status)||0};
  var arr=r.info||r.items||[];
  window.__koeInfoItems={};
  function looksUrl(v){return typeof v==="string"&&/^https?:\/\//i.test(v.trim())}
  return {ok:true,notifications:arr.map(function(x){
    ...
    var it={id:x.id,type:"info",info_detail:true,
      title:title, body:body, url:url,
      message:title+(body?" — "+String(body).replace(/\s+/g," ").slice(0,60)+(String(body).length>60?"…":""):""),
      created_at:x.updated_at||x.created_at||x.createdAt||x.published_at||"",
      user_id:0,name:"運営からのお知らせ",icon_url:""};
    try{window.__koeInfoItems[String(x.id)]=it}catch(e){}
    return it;})};
}
/* 運営からのお知らせの全文を出す。 */
/* 汎用: 要素の上で左右スワイプしたら、タブを前後に動かす。 */
function koeAttachTabSwipe(el, isActive, prev, next){
  if(!el||el.__koeTabSwipe) return; el.__koeTabSwipe=true;
  var x0=0,y0=0,t0=0,tracking=false;
  el.addEventListener("touchstart",function(e){
    if(!e.touches||e.touches.length!==1||!isActive()){ tracking=false; return; }
    x0=e.touches[0].clientX; y0=e.touches[0].clientY; t0=Date.now(); tracking=true;
  },{passive:true});
  el.addEventListener("touchend",function(e){
    if(!tracking) return; tracking=false; if(!isActive()) return;
    var tt=e.changedTouches[0]; if(!tt) return;
    var dx=tt.clientX-x0, dy=tt.clientY-y0, dt=Date.now()-t0;
    if(dt>500) return;
    if(Math.abs(dx)<70) return;
    if(Math.abs(dx)<Math.abs(dy)*2.0) return;
    try{ if(dx<0) next(); else prev(); try{haptic(8)}catch(e){} }catch(e){}
  },{passive:true});
}
(function(){
  function wire(){
    ...
    if(np) koeAttachTabSwipe(np,
      function(){ return np.classList.contains("active"); },
      function(){ koeNotifStep(-1); }, function(){ koeNotifStep(1); });
    // 友達モーダル: 相互/友達 を左右で切り替え
    var fm=document.getElementById("followListModal");
    if(fm) koeAttachTabSwipe(fm,
      function(){ return fm.style.display!=="none" && window.__koeFlKind==="friends"; },
      function(){ koeFlStep(-1); }, function(){ koeFlStep(1); });
  }
  function koeNotifStep(dir){
    try{
      var chips=[].slice.call(document.querySelectorAll(".notif-kind-chip")).filter(function(c){return c.style.display!=="none"});
      var i=chips.findIndex(function(c){return c.classList.contains("active")});
      if(i<0)i=0; var ni=i+dir; if(ni<0||ni>=chips.length)return;
      chips[ni].click();
    }catch(e){}
  }
  window.koeFlStep=function(dir){
    try{
      ...
      var ni=order.indexOf(cur)+dir; if(ni<0||ni>=order.length)return;
...
function koeShowInfoDetail(it){
  try{sfx("open")}catch(e){}
  var old=document.getElementById("koeInfoModal"); if(old)old.remove();
  var when="";
  try{ when=(typeof koeTimeLabel==="function"&&it.created_at)?koeTimeLabel(it.created_at):(it.created_at||""); }catch(e){}
  var hasBody=!!(it.body&&String(it.body).trim());
  var bodyHtml=hasBody?escapeHtml(it.body).replace(/\n/g,"<br>")
                      :(it.url?'このお知らせの詳細はWEBページで公開されています。下のボタンから開いてください。':'(本文なし)');
  /* URL は属性に直接埋め込まない(引用符で属性が壊れる/属性注入の恐れ)。描画後に JS 側で紐づける。 */
  var linkHtml=(it.url&&/^https?:\/\//i.test(String(it.url)))?'<a href="#" class="info-link" id="koeInfoLink">詳細を開く</a>':"";
  var html='<div id="koeInfoModal" class="modal" style="display:flex;">'
    +'<div class="modal-content info-modal">'
    +'<div class="modal-head"><h3 style="margin:0;flex:1;">'+escapeHtml(it.title||"お知らせ")+'</h3>'
  ...
  try{ var lk=document.getElementById("koeInfoLink"); if(lk){ var u=String(it.url); lk.addEventListener("click",function(ev){ try{ev.preventDefault(); if(window.AndroidApi&&window.AndroidApi.openUrl)window.AndroidApi.openUrl(u); else window.open(u,"_blank"); }catch(e){} }); } }catch(e){}
  try{ if(window.__koeStackFix)window.__koeStackFix(); }catch(e){}
}
function koeInGroupCall(){
  try{ return !!((typeof skRoom!=="undefined"&&skRoom)||(typeof skCurrentRoomId!=="undefined"&&skCurrentRoomId)||document.body.classList.contains("in-call")); }catch(e){ return false; }
}
/* 通話中にホーム等へ移動したら小窓(設定の表示方法)を出す。戻ってきたら消す。 */
(function(){
  if(window.__koeCallBgHook) return; window.__koeCallBgHook=true;
  function onHide(){
    try{
  ...
  function onShow(){
    try{
      ...
      if(koeInGroupCall()&&ov&&ov.style.display==="none"){ /* 小窓から戻ったら操作はユーザーに任せる */ }
    }catch(e){}
  }
  try{
    document.addEventListener("visibilitychange",function(){ if(document.hidden)onHide(); else onShow(); });
    window.addEventListener("pagehide",onHide);
    window.addEventListener("blur",function(){ /* blur は誤爆しやすいので少し待って隠れているか確認 */
      setTimeout(function(){ if(document.hidden)onHide(); },350);
    });
  }catch(e){}
})();
function koeFollowPillHtml(uid,state){
  var cls="fl-pill "+(state==="following"?"is-on":state==="requested"?"is-wait":"is-off");
  var label=state==="following"
      ...
      :'フォロー';
  return '<button type="button" class="'+cls+'" data-uid="'+uid+'" data-state="'+state+'" onclick="koeFollowPillTap(event,'+uid+')">'+label+'</button>';
}
/* 「相互フォロー」と「友達」はkoetomoでは別の関係なので、上のタブで切り替える。 */
function koeIsMutual(u){return !!(u&&u.is_following&&u.is_followed)}
function koeIsFriend(u){return !!(u&&u.is_friend)}
function koeFlTabsHtml(us){
  us=us||[];
  var m=us.filter(koeIsMutual).length, f=us.filter(koeIsFriend).length;
  var t=window.__koeFlTab||"mutual";
  return '<div class="fl-tabs">'
    +'<button type="button" class="fl-tab'+(t==="mutual"?" active":"")+'" onclick="koeSwitchFlTab(\'mutual\')">相互フォロー <span class="fl-tabn">'+m+'</span></button>'
    +'<button type="button" class="fl-tab'+(t==="friends"?" active":"")+'" onclick="koeSwitchFlTab(\'friends\')">友達 <span class="fl-tabn">'+f+'</span></button>'
    +'</div>';
}
...
function koeRenderFollowRows(q){
  var box=document.getElementById("followListRows"); if(!box)return;
  var us=window.__koeFollowListUsers||[];
  q=(q||"").trim().toLowerCase();
  var list=q?us.filter(function(u){return String(u.name||"").toLowerCase().indexOf(q)>=0||String(u.user_id||"").indexOf(q)>=0}):us.slice();
  var empty="見つかりません";
  ...
  box.innerHTML=list.length?list.map(koeFollowRowHtml).join(""):'<div class="empty-msg">'+empty+'</div>';
  try{
    var tb=document.querySelectorAll("#followListBody .fl-tabn");
    if(tb.length===2){tb[0].textContent=us.filter(koeIsMutual).length;tb[1].textContent=us.filter(koeIsFriend).length}
  }catch(e){}
}
...
async function koeFollowPillTap(ev,uid){
  ...
  btn.disabled=true;
  try{haptic(10)}catch(e){}
  var r;try{r=await callApi(willFollow?"follow_user":"unfollow_user",uid)}catch(e){r=null}
  btn.disabled=false;
  if(r&&r.ok){
    try{window.dispatchEvent(new CustomEvent("koe:follow-changed",{detail:{userId:uid,following:willFollow}}))}catch(e){}
    try{window.__koeFriendCount=null;koeLoadFriendCount()}catch(e){}
    try{ (window.__koeFollowListUsers||[]).forEach(function(u){ if(String(u.user_id)===String(uid)){u.is_following=willFollow;u.requested=false} }) }catch(e){}
    try{sfx(willFollow?"follow":"unlike")}catch(e){}
  }else{
    koeSetFollowUi(uid,was);
    try{toast((r&&r.message)?String(r.message):(willFollow?"フォローできませんでした":"フォロー解除できませんでした"),"error")}catch(e){}
  }
}
...
function koeListPublic(p,kind){
  try{
    ...
    var key=kind==="follower"?"is_follower_list_public":kind==="followee"?"is_follow_list_public":"is_friend_list_public";
    if(!(key in st))return true;
    return !(st[key]===false||Number(st[key])===0);
  }catch(e){return true}
}
var KOE_LOCK_SVG='<svg class="ico koe-lock" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4.5" y="10.5" width="15" height="10" rx="2"/><path d="M8 10.5V7a4 4 0 0 1 8 0v3.5"/></svg>';
function koeStatHtml(p,kind,uid,count,label){
  ...
  return '<span class="pv-stat-click'+(pub?"":" pv-stat-lock")+'"'
    +(pub?"":' title="'+escAttr(label)+'は非公開に設定されています"')
    +' onclick=\'openFollowList('+(Number(uid)||0)+', "'+listKind+'")\'>'
    +lock+'<b>'+fmtNum(count)+'</b> '+escapeHtml(label)+'</span>';
}
function updateFollowButton(){
    ...
    btn.textContent=profileViewFollowState.following?"フォロー中(解除)":pending?"フォローを返す":"フォローする";
...
function koeUpdateFriendButton(){
  try{
    ...
    if(!uid||String(uid)===String(me)){ if(b)b.style.display="none"; return }
    if(!b){
    ...
    b.textContent=st==="friend"?"友達(解除)":st==="sent"?"友達申請中(取り消す)":st==="in"?"友達申請を承認する":"友達申請する";
...
if(loggedIn&&loggedIn.logged_in){koeAuthRelease();enterMain(loggedIn.user_name);}else{
  ...
  koeAuthRelease();
  if(!__autoOk){window.__koeShellShown=false;window.__koeShellTab=null;showScreen("loginScreen");}
}});
/* 認証待ちの解除。どの経路を通っても必ず呼ぶ(呼び忘れると全ての通信が止まる) */
function koeAuthRelease(){try{if(window.__koeAuthDone){window.__koeAuthDone()}}catch(e){}window.__koeAuthGate=null;window.__koeAuthDone=null;}
...
function koeBootShell(acc){
  try{
    ...
    window.__koeAuthGate=new Promise(function(res){window.__koeAuthDone=res});
    /* 何があっても通信が止まったままにならないよう、8秒で必ず解除する */
    try{ setTimeout(koeAuthRelease,8000) }catch(e){}
    var hu=document.getElementById("headerUser"); if(hu&&acc&&acc.name)hu.textContent=acc.name;
    showScreen("mainScreen");
    var tab="timeline"; try{tab=(window.__koeStartTab?window.__koeStartTab():(localStorage.getItem("koe_last_tab")||"timeline"))}catch(e){}
    window.__koeShellTab=tab;
    var list=document.getElementById("timelineList");
    if(list&&!list.innerHTML){ var c=null; try{c=__tlCacheGet("all")}catch(e){} list.innerHTML=c||skeletonCards(4); }
    try{showPage(tab)}catch(e){}
    return true;
  }catch(e){ return false }
}
...
try{if(window.__myUserId)return String(window.__myUserId)}catch(e){}
try{if(typeof currentAccountId==="function"){var a=currentAccountId();if(a)return String(a)}}catch(e){}
return "";
}
...
toastMsg(r&&r.ok?("削除しました"+(r.deleted?"("+r.deleted+"件)":"")):((r&&r.message)?String(r.message):"削除に失敗しました"),r&&r.ok?undefined:"error");b.disabled=false;
...
try{if(window.AndroidApi&&window.AndroidApi.nativeLog){r=JSON.parse(window.AndroidApi.nativeLog())}}catch(e){r=null}
if(!r){try{r=await Promise.race([window.pywebview.api.get_native_log(),new Promise(function(res){setTimeout(function(){res({ok:false,error:"timeout(3s)"})},3000)})])}catch(e){r={ok:false,error:String(e&&e.message||e)}}}
if(!r||!r.ok||!r.log){window.__koeNativeLogErr="(取得失敗: "+(r&&r.error||"応答なし")+")";return}
...
const __bv=(function(){try{var j=window.AndroidApi&&window.AndroidApi.appVersion&&window.AndroidApi.appVersion();if(!j)return"";var o=JSON.parse(j);return o&&o.name?(o.name+(o.code?(" ("+o.code+")"):"")):""}catch(e){return""}})();return "=== KoeTomo 診断ログ ===\nbuild: "+(__bv?("v"+String(__bv).replace(/^v/,"")):"(不明)")+"\n[JS/API]\n"+js.join("\n")+"\n\n[ネイティブHTTP]\n"+(nat.length?nat.join("\n"):(window.__koeNativeLogErr||"(0件)"));
}
...
(function(){
  ...
  function openExt(url){if(!/^https?:\/\//i.test(String(url||"")))return;try{if(window.AndroidApi&&window.AndroidApi.openUrl){window.AndroidApi.openUrl(url);return}}catch(e){}try{window.open(url,"_blank")}catch(e2){}}
...
(function(){
  ...
  function hookNavVisSection(){
    try{
      ...
      var det=box.closest("details"); if(det&&!det.__navHook){ det.__navHook=true;
        det.addEventListener("toggle",function(){ if(det.open){ try{renderNavVisibility();renderLandingSel();}catch(e){} } });
      }
    }catch(e){}
  }
  function renderNavVisibility(){
    ...
    Array.prototype.forEach.call(box.querySelectorAll('input[type=checkbox]'),function(cb){
      cb.addEventListener("change",function(){
        ...
        try{ if(typeof applyNavOrder==="function") applyNavOrder(); }catch(e){}
        try{ toast(cb.checked?(label(v)+"を表示しました"):(label(v)+"を非表示にしました")); }catch(e){}
      });
    });
  }
  ...
  function boot(){
    applyNavHidden(); renderLandingSel(); renderNavVisibility(); hookNavVisSection(); hookSections(); persistFeed();
  ...
  async function koeSend(){
    ...
    if(pendingImg){
      ...
      try{
        ...
        if(r&&r.ok){ if(text){ try{await callApi("send_message",currentChat.chatId,currentChat.targetId,text);}catch(e){} } if(ta){ta.value="";grow(ta);} try{sfx("send")}catch(e){} await reloadMessages(true); setTimeout(function(){reloadMessages(true)},1200); setTimeout(function(){reloadMessages(true)},3000); }
...
(function(){
  ...
  function applyUserFont(dataUrl){
    try{
      ...
      ff.load().then(function(loaded){ try{document.fonts.add(loaded); if(!window.__koeUserFontOn)return; if(window.koeSetFontFamily){koeSetFontFamily("'KoeUserFont','Hiragino Sans','Noto Sans JP',sans-serif")}else{document.body.style.fontFamily="'KoeUserFont', sans-serif"}}catch(e){} }).catch(function(){ window.__koeUserFontOn=false; toast&&toast("フォントを読み込めませんでした","error"); });
    }catch(e){}
  }
  function init(){
    ...
    window.koeClearUserFont=function(){ window.__koeUserFontOn=false; try{localStorage.removeItem(FKEY);localStorage.removeItem(FNAME);}catch(e){} };
...
(function(){
  ...
  function doSave(src){
    ...
    try{ if(typeof showImgFormat==="function"){ showImgFormat(src); return; } }catch(e){}
...
(function(){
  ...
  async function doProfileReport(ev){
    ...
    var st=(typeof profileViewFollowState!=="undefined"&&profileViewFollowState)?profileViewFollowState:window.profileViewFollowState; if(!st){ try{toast("対象ユーザーが不明です","error")}catch(e){} return; }
    var uid=st.userId;
    ...
    T((r0&&r0.ok)?"運営に通報しました":("通報に失敗しました"+(r0&&r0.status?" (status "+r0.status+")":"")),(r0&&r0.ok)?undefined:"error");
    return;
  }
  ...
  window.__koeSubmitBlacklistReport=async function(uid){ try{ await doBlacklistReport(uid); }catch(e){ T("ブラックリスト申請に失敗しました","error"); } };
...
(function(){
  ...
  window.addEventListener("koe:block-changed",function(){refreshOpenLists(false)});
  window.addEventListener("koe:follow-changed",function(){refreshOpenLists(true)});
})();
...
(function(){
  ...
  async function loadMyAlbum(){
    ...
    box.innerHTML=withImg.map(function(p){
      return '<img loading="lazy" decoding="async" src="'+escAttr(p.image_url)+'" onclick="openPostDetail(event,'+(Number(p.id)||0)+')" onerror="this.style.opacity=\'.15\'">';
    }).join("");
  }
...
function koeQuickEditPreview(){
  try{
    var modal=document.getElementById("profileQuickEditModal"); if(!modal)return;
    ...
    var icon="";
    try{
      var im=document.querySelector("#profileViewAvatar img.avatar")||document.querySelector("#myProfileAvatar img.avatar");
      if(im)icon=im.src;
    }catch(e){}
    box.innerHTML='<div class="qe-preview-label">保存後はこう見えます</div>'
      +'<div class="qe-preview-card">'
        +(typeof avatarHtml==="function"?avatarHtml(name||"?",icon):"")
        +'<div class="qe-preview-body">'
          +'<div class="qe-preview-name">'+escapeHtml(name||"(名前なし)")+'</div>'
          +'<div class="qe-preview-comment">'+(cmt?escapeHtml(cmt):'<span style="opacity:.5">(自己紹介なし)</span>')+'</div>'
        +'</div>'
      +'</div>';
  }catch(e){}
}
(function(){
  function bind(){
    ["quickEditName","quickEditComment"].forEach(function(id){
      var el=document.getElementById(id);
      if(el&&!el.__koePv){el.__koePv=1;el.addEventListener("input",koeQuickEditPreview)}
    });
  }
  if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",bind)}else{bind()}
  setTimeout(bind,1500);
})();
function openQuickEdit(){try{setTimeout(koeQuickEditPreview,0)}catch(e){}
...
(function(){
  ...
  var NOTIF_TABS=[{key:"normal",label:"通常"},{key:"important",label:"重要"},{key:"calls",label:"着信"},{key:"info",label:"お知らせ"}];
...
window.koeIsWebUrl=function(u){ try{ return /^https?:\/\//i.test(String(u||"").trim()) }catch(e){ return false } };
window.koeOpenExternal=function(url){
  if(!window.koeIsWebUrl(url)){ try{ if(typeof toast==="function") toast("開けないリンクです","error") }catch(e){} return; }
  try{ if(typeof toast==="function") toast("リンクを開きます: "+String(url).replace(/^https?:\/\//,"").slice(0,40)); }catch(e){}
  try{ if(window.AndroidApi&&window.AndroidApi.openUrl){ window.AndroidApi.openUrl(url); return; } }catch(e){}
  try{ window.open(url,"_blank"); }catch(e){}
};
// プロフィール等の本文リンク：タップで開く（結果が分かるようトースト）／長押しでコピー
(function(){
  if(window.__koeLinkFeedback) return; window.__koeLinkFeedback=true;
  document.addEventListener("click",function(e){
  ...
  document.addEventListener("touchstart",function(e){
    ...
    lt=setTimeout(function(){
      try{ navigator.clipboard.writeText(a.getAttribute("href")); if(typeof toast==="function") toast("リンクをコピーしました"); if(typeof haptic==="function") haptic(12); }catch(err){}
      lt=null;
    },600);
  },{passive:true});
...
(function(){
  ...
  function startUpdate(r){
    ...
    if(!/^https:\/\/(github\.com\/haizarakun\/koetomoProject\/|objects\.githubusercontent\.com\/|release-assets\.githubusercontent\.com\/)/i.test(String(r.apk||""))){ T("更新ファイルの場所が不正です","error"); try{window.koeOpenExternal(RELEASES);}catch(x){} return; }
...
(function(){
  if(window.__koeRoomsMore) return; window.__koeRoomsMore=true;
  ...
  window.renderRooms=function(rooms){
    ...
    try{
      ...
      if(list&&list.querySelector(".room-card")&&!document.getElementById("roomsMoreBtn")){
        var b=document.createElement("button");
        b.id="roomsMoreBtn"; b.className="btn-secondary"; b.style.marginTop="10px"; b.textContent="もっと見る";
        b.addEventListener("click",function(){ koeLoadMoreRooms(); });
        list.appendChild(b);
      }
    }catch(e){}
    return r;
...
try{ document.documentElement.classList.add("koe-boot");
  var __rmBoot=function(){ try{document.documentElement.classList.remove("koe-boot")}catch(e){} };
  if(window.requestAnimationFrame){ requestAnimationFrame(function(){ requestAnimationFrame(function(){ setTimeout(__rmBoot,120) }) }); }
  else setTimeout(__rmBoot,300);
}catch(e){}
(function(){
if(window.__koeRandomInit)return; window.__koeRandomInit=true;
var S={on:false,phase:"",mid:0,eid:0,target:0,me:0,token:"",lastMid:0,t0:0,deadline:0,tick:null,busy:false,
        waitingAnswer:false,showingDialog:false,matchedUserId:0,rtdbNg:0,lastBeep:-1};

function el(){ return document.getElementById("koeRandomModal"); }
function T(msg,kind){ try{toast(msg,kind)}catch(e){} }
function stopTick(){ if(S.tick){ try{clearInterval(S.tick)}catch(e){} S.tick=null; } streamStop(); try{koeMgStop()}catch(e){} }

function ensureModal(){
  var m=el(); if(m) return m;
  m=document.createElement("div"); m.className="modal"; m.id="koeRandomModal"; m.style.display="flex";
  m.innerHTML='<div class="modal-content small"><div class="modal-header"><span>ランダム通話</span><button class="modal-close koe-rnd-x">✕</button></div><div class="modal-body" id="koeRandomBody"></div></div>';
  document.body.appendChild(m);
  m.querySelector(".koe-rnd-x").addEventListener("click",function(){ finish(true) });
  return m;
}
function body(html){ ensureModal(); var b=document.getElementById("koeRandomBody"); if(b)b.innerHTML=html; wire(); }
function wire(){
  var m=el(); if(!m)return;
  var c=m.querySelector(".koe-rnd-cancel"); if(c)c.addEventListener("click",function(){ finish(true) });
  var ok=m.querySelector(".koe-rnd-ok"); if(ok)ok.addEventListener("click",acceptMatching);
  var sk=m.querySelector(".koe-rnd-skip"); if(sk)sk.addEventListener("click",function(){ refuseMatching(true) });
}
function renderSearching(){
  var sec=Math.max(0,Math.round((Date.now()-S.t0)/1000));
  var secEl=document.getElementById("koeRndSec");
  if(secEl){ secEl.textContent="経過 "+sec+" 秒"; return; }   /* 毎秒作り直すとゲームが消えるので、時間だけ更新 */
  body('<div style="text-align:center;padding:12px 6px 4px;">'
    +'<div class="joining-spinner" style="margin:0 auto 10px;"></div>'
    +'<div style="font-weight:700;font-size:15px;">通話相手をさがしています…</div>'
    +'<div class="card-sub" style="margin-top:4px;" id="koeRndSec">経過 '+sec+' 秒</div>'
    +'<div class="koe-mg-wrap" style="margin-top:12px;">'
      +'<div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;opacity:.8;margin-bottom:4px;">'
        +'<span>待ち時間に一息</span><span id="koeMgInfo">最高 '+koeMgBest()+'</span></div>'
  ...
  try{ koeMgStart() }catch(e){}
}

/* ===== 待ち時間の極小ミニゲーム: 光ったマスをタップ =====
   検索中だけ動き、マッチ・画面を閉じた時に止まる。DOMだけで実装(約0.8KB) */
function koeMgBest(){ try{ return parseInt(localStorage.getItem("koe_mg_best")||"0",10)||0 }catch(e){ return 0 } }
function koeMgStop(){ try{ clearTimeout((window.__koeMg||{}).t) }catch(e){} window.__koeMg=null; }
function koeMgStart(){
  koeMgStop();
  var g=document.getElementById("koeMgGrid"), msg=document.getElementById("koeMgMsg"), info=document.getElementById("koeMgInfo");
  if(!g)return;
  var G={sc:0,ms:1600,life:3,cur:-1,t:0}; window.__koeMg=G;
  g.innerHTML=""; var cells=[];
  for(var i=0;i<9;i++){ var d=document.createElement("div"); d.className="koe-mg-c"; d.setAttribute("data-i",i); g.appendChild(d); cells.push(d); }
  function info2(){ if(info)info.textContent="スコア "+G.sc+" ／ 最高 "+Math.max(G.sc,koeMgBest());
    if(msg)msg.textContent="のこりミス "+G.life+" 回"; }
  function end(){ var b=koeMgBest(); if(G.sc>b){try{localStorage.setItem("koe_mg_best",String(G.sc))}catch(e){}}
    koeMgStop(); cells.forEach(function(c){c.classList.remove("on")});
    if(msg)msg.textContent="スコア "+G.sc+" ・ タップでもう一度";
    g.onclick=function(){ koeMgStart() }; }
  function miss(){ if(!window.__koeMg)return; G.life--; try{koeVibrate(20)}catch(_){}
    if(G.life<=0)return end(); info2(); next(); }
  function next(){
    if(!window.__koeMg||S.phase!=="searching")return koeMgStop();
    cells.forEach(function(c){c.classList.remove("on")});
    G.cur=(Math.random()*9)|0; cells[G.cur].classList.add("on");
    clearTimeout(G.t); G.t=setTimeout(miss,G.ms);
  }
  g.onclick=function(e){
    if(!window.__koeMg)return;
    var c=e.target&&e.target.classList&&e.target.classList.contains("koe-mg-c")?e.target:null; if(!c)return;
    if(+c.getAttribute("data-i")===G.cur){ clearTimeout(G.t); G.sc++; G.ms=Math.max(520,G.ms-45);
      try{sfx("like")}catch(_){} try{koeVibrate(8)}catch(_){} info2(); next(); }
    else miss();
  };
  info2(); next();
}
...
function renderConnecting(msg){
  body('<div style="text-align:center;padding:18px 6px;">'
    +'<div class="joining-spinner" style="margin:0 auto 14px;"></div>'
    +'<div style="font-weight:700;font-size:15px;">'+escapeHtml(msg||"通話に接続しています…")+'</div>'
...
function renderMatched(p){
  ...
  body('<div style="text-align:center;padding:6px 4px 2px;">'
    +'<div style="font-weight:700;font-size:15px;margin-bottom:10px;">相手が見つかりました</div>'
    +'<div style="display:flex;justify-content:center;margin-bottom:8px;">'+avatarHtml(p.name,p.icon_url)+'</div>'
    +'<div style="font-weight:700;font-size:16px;">'+escapeHtml(p.name||("user "+S.target))+'</div>'
    +(meta.length?'<div class="card-sub" style="margin-top:4px;">'+meta.join(" ・ ")+'</div>':'')
    +(p.comment?'<div class="card-sub" style="margin-top:8px;white-space:pre-wrap;text-align:left;max-height:88px;overflow:auto;">'+escapeHtml(p.comment)+'</div>':'')
...
async function start(){
  if(S.on)return;
  S.on=true; S.phase="searching"; S.mid=0; S.eid=0; S.target=0; S.token=""; S.matchedUserId=0;
  S.waitingAnswer=false; S.showingDialog=false; S.rtdbNg=0; S.t0=Date.now();
  ensureModal(); renderSearching();
  ...
  try{
    ...
    if(sv===false||sv===0||sv==="0"){
      if(await showConfirmModal("ランダムマッチングの設定がオフになっています。オンにして相手をさがしますか？")){
        await callApi("set_user_settings",JSON.stringify({random_match_enabled:true}));
      }else{ S.on=false; var m0=el(); if(m0)m0.remove(); return; }
    }
  }catch(e){}
  if(!S.on)return;
  var r=await callApi("matching_start","1");
  if(!r||!r.ok){ T("マッチングを開始できませんでした"+(r&&(r.message||r.error)?"："+(r.message||r.error):""),"error"); S.on=false; var m1=el(); if(m1)m1.remove(); return; }
  try{sfx("open")}catch(e){}
  /* 公式は Firebase のリアルタイム通知(ValueEventListener)で即座に気づく。こちらも同じく
     matching_info を購読して、変化があった瞬間に評価する（届かない環境では下の定期確認が拾う）。 */
  S.tick=setInterval(function(){loop(false)},1000);
...
function streamStart(){
  try{
    ...
    var es=new EventSource("https://koetomo-bb8bb.firebaseio.com/users/"+encodeURIComponent(String(S.me))+"/matching_info.json");
    S.es=es;
    var hit=function(){ S.streamAlive=Date.now(); if(S.on) loop(true); };
    es.addEventListener("put",hit);
    es.addEventListener("patch",hit);
    es.addEventListener("keep-alive",function(){ S.streamAlive=Date.now(); });
    es.addEventListener("cancel",function(){ streamStop(); });
    es.onerror=function(){ streamStop(); if(S.on) setTimeout(streamStart,4000); };
  }catch(e){}
}
function streamStop(){ try{ if(S.es)S.es.close() }catch(e){} S.es=null; }

/* onPause 相当: ダイアログ表示中なら見送りを送り、待ち行列から抜けて画面を閉じる */
async function finish(userAction){
  if(!S.on){ var m=el(); if(m)m.remove(); return; }
  var wasDialog=S.showingDialog, mid=S.mid, eid=S.eid;
  S.on=false; S.phase=""; S.showingDialog=false; S.waitingAnswer=false;
  stopTick(); var mm=el(); if(mm)mm.remove();
  if(wasDialog&&mid){ S.lastMid=mid; try{ await callApi("matching_refuse",String(mid),String(eid)) }catch(e){} }
  try{ await callApi("matching_cancel") }catch(e){}
  if(S.token){ try{ await callApi("dive_request_cancel") }catch(e){} }
  S.token=""; S.target=0;
...
async function loop(fromStream){
  if(!S.on||S.busy)return; S.busy=true;
  try{
    if(S.phase==="matched"){
      var left=Math.max(0,Math.round((S.deadline-Date.now())/1000));
      var bar=document.getElementById("koeRndBar"), lb=document.getElementById("koeRndLeft");
      if(bar)bar.style.width=Math.round(left/61*100)+"%";
      if(lb)lb.textContent=left+"秒";
      /* 公式は5秒ごとに音とバイブで知らせる */
      if(left%5===0&&left!==S.lastBeep&&left>0){ S.lastBeep=left; try{sfx("notify")}catch(e){} try{koeVibrate(500)}catch(e){} }
      if(left<=0){ S.busy=false; refuseMatching(false); return; }
      S.busy=false; return;
    }
    /* 通信の間引き: リアルタイム通知が生きている間は保険の定期確認だけにする */
    var now=Date.now(), live=(S.streamAlive&&now-S.streamAlive<60000);
    if(!fromStream && now-(S.lastCheck||0) < (live?6000:1200)){ S.busy=false; return; }
    S.lastCheck=now;
    var st=await callApi("matching_state");
    if(!st||!st.ok){ S.busy=false; return; }
    S.me=st.my_user_id||S.me;
    S.rtdbNg=(st.rtdb&&st.rtdb!=="ok")?(S.rtdbNg+1):0;
    if(S.rtdbNg===6){ T(st.rtdb==="denied"?"マッチング通知の受信が拒否されました。しばらくしてからお試しください":"マッチング通知サーバーに接続できません。電波の良い場所でお試しください","error"); }
    var mid=st.matching_id||0;
    if(mid){ S.mid=mid; S.eid=st.entry_id||S.eid; }
    if(st.state==="mutual"){
      /* 公式: |now - mutual_accepted_at| <= 15秒 のときだけ通話へ進む */
      if(st.mutual_fresh){
        S.target=st.target_id||S.target; S.matchedUserId=S.target;
        S.busy=false; connect(); return;
      }
    }else if(st.state==="matched" && mid!==S.lastMid){
      /* 公式: now < matched_at + 15秒 かつ ダイアログ非表示のときだけ相手を表示 */
      if(st.matched_fresh && !S.showingDialog && S.phase==="searching"){
        S.target=st.target_id||0; S.showingDialog=true; S.phase="matched";
        S.deadline=Date.now()+61000; S.lastBeep=-1;
        try{koeVibrate(500)}catch(e){}
        /* まずカードを出してから、相手の詳細を後から差し替える（表示を待たせない） */
        renderMatched({name:"",icon_url:"",user_id:S.target});
        var __m=S.mid;
        Promise.resolve(callApi("view_user_profile",String(S.target))).then(function(pr){
          try{
...
async function acceptMatching(){
  ...
  S.showingDialog=false; S.waitingAnswer=true; S.phase="waiting"; renderWaiting();
  var r=await callApi("matching_accept",String(S.mid),String(S.eid));
  if(!r||!r.ok){ T("承諾を送れませんでした","error"); finish(false); }
}

/* refuseMatching 相当（見送り／61秒の時間切れ。公式はどちらも画面を閉じる） */
async function refuseMatching(byUser){
  if(S.phase!=="matched")return;
  var mid=S.mid, eid=S.eid;
  S.lastMid=mid; S.showingDialog=false; S.phase=""; S.on=false;
  stopTick(); var m=el(); if(m)m.remove();
  try{ await callApi("matching_refuse",String(mid),String(eid)) }catch(e){}
  try{ await callApi("matching_cancel") }catch(e){}
  T("マッチングを見送りました");
}
...
async function connect(){
  ...
  S.phase="connecting"; S.waitingAnswer=false; stopTick();
  try{ await callApi("matching_cancel") }catch(e){}
  ...
  try{
    var token="";
    if(caller){
      if(S.lastMid===S.mid&&S.token){ /* 二重発信の防止(公式の lastMatchingId と同じ) */ }
      S.lastMid=S.mid;
      var dr=await callApi("dive_request",String(S.target));
      if(!dr||!dr.ok||!dr.token) throw new Error((dr&&(dr.message||dr.error))||"発信できませんでした");
      S.token=dr.token;
      var ready=false;
      for(var i=0;i<40&&S.phase==="connecting";i++){
        var cs=await callApi("dive_confirm_status",S.token);
        if(cs&&cs.ok&&cs.ready){ ready=true; break; }
      ...
      for(var k=0;k<15&&S.phase==="connecting";k++){
        ...
        if(cf&&cf.ok&&cf.token){ token=cf.token; break; }
        await new Promise(function(r){setTimeout(r,1000)});
      }
      ...
      for(var j=0;j<40&&S.phase==="connecting";j++){
        ...
        if(ic&&ic.ok&&ic.incoming){
          ...
          if(ck&&ck.ok&&ck.has_request&&(!ck.target_id||!S.matchedUserId||Number(ck.target_id)===Number(S.matchedUserId))){ got=ck; break; }
        }
        await new Promise(function(r){setTimeout(r,1000)});
      }
      ...
      if(!rc||!rc.ok||!rc.token) throw new Error((rc&&(rc.message||rc.error))||"通話を受けられませんでした");
      token=rc.token;
    }
    if(S.phase!=="connecting")return;
    S.token=token;
    ...
    window.__koeDiveActive={target:String(S.target),token:String(token),startedAt:Date.now(),name:(S.matchedName||"")};
    var m2=el(); if(m2)m2.remove();
    S.on=false; S.phase="";
    try{sfx("join")}catch(e){}
    await startInWindowCall(vc.call);
  }catch(e){
    T(String((e&&e.message)||e||"接続に失敗しました"),"error");
    try{ if(S.target) await callApi("dive_request_disconnect",String(S.target),"connect_failed") }catch(_){}
...
try{
  if(typeof window.teardownCall==="function"&&!window.__koeDiveTeardown){
    ...
    window.teardownCall=async function(notify){
      var a=window.__koeDiveActive; window.__koeDiveActive=null;
      var sec=0; try{ if(a&&a.startedAt) sec=Math.round((Date.now()-a.startedAt)/1000) }catch(e){}
      var r=await _tc.apply(this,arguments);
      if(a&&a.target){ try{ await callApi("dive_request_disconnect",String(a.target),"") }catch(e){} }
      if(a&&a.token) setTimeout(function(){ try{ koeAskGoodTalk(a,sec) }catch(e){} },600);
      return r;
    };
  }
}catch(e){}
...
async function koeAskGoodTalk(a,sec){
  ...
  var min=60; try{ var g=await callApi("get_good_talk_min"); if(g&&g.ok&&g.min_second) min=Number(g.min_second)||60 }catch(e){}
  if(sec<min)return;
  if(document.getElementById("koeGoodTalkModal"))return;
  var d=document.createElement("div"); d.id="koeGoodTalkModal"; d.className="modal"; d.style.display="flex";
  d.innerHTML='<div class="modal-content small"><div class="modal-header"><h3>グッドトーク</h3></div>'+
    '<div class="modal-body"><p style="font-size:13px;line-height:1.6;margin:0;">'+(a.name?escapeHtml(a.name)+'さんとの':'')+'いまの通話はどうでしたか？<br>よかったら「グッドトーク」を送れます（相手に高評価が付きます）。</p></div>'+
  ...
  d.querySelector("#koeGtYes").addEventListener("click",async function(){
    ...
    var r=null; try{ r=await callApi("dive_like",String(a.token)) }catch(e){}
    close();
    if(r&&r.ok){ window.__koeGoodTalkSent=a.token; T("グッドトークを送りました"); try{sfx("like")}catch(e){} }
    else T("送れませんでした"+(r&&r.error?"（"+r.error+"）":""),"error");
  });
}
window.koeAskGoodTalk=koeAskGoodTalk;
window.__setRoomForTest=function(rid,owner){try{currentRoomId=rid;currentRoomOwnerId=owner}catch(e){}};
window.koeRandomCall=start;
function bindBtn(){ try{ var b=document.getElementById("koeRandomBtn"); if(b&&!b.__b){ b.__b=1; b.addEventListener("click",function(){ start() }); } }catch(e){} }
document.addEventListener("DOMContentLoaded",bindBtn); bindBtn();
})();
...
(function(){
if(window.__koeRoomCtl)return; window.__koeRoomCtl=true;
...
function koeRosterRemember(r){
  ...
  [].concat(r.speakers||[],r.listeners||[],r.speaker_applicants||[]).forEach(function(u){
    var id=Number(u.user_id||u.userId); if(!id)return;
...
function koeMergeRoster(res){
  try{
    ...
    if(res&&res.__src==="rtdb"){ S.rtdb=res; S.rtdbAt=Date.now(); } else if(res){ S.rest=res; }
    koeRosterRemember(S.rest); koeRosterRemember(S.rtdb);
    var rt=S.rtdb, n=rt?((rt.speakers||[]).length+(rt.listeners||[]).length+(rt.speaker_applicants||[]).length):0;
    var base=(rt && n>0 && (Date.now()-S.rtdbAt)<20000) ? rt : (S.rest||res||{});
    var fill=function(k){ return (base[k]||[]).map(function(u){
        var id=Number(u.user_id||u.userId), nm=S.names[id]||{};
        return {user_id:id,
                name:(u.name&&!/^user \d+$/.test(u.name))?u.name:(nm.name||u.name||("user "+id)),
                icon_url:u.icon_url||nm.icon_url||"", is_mute:u.is_mute};
      }).filter(function(u){return !!u.user_id}); };
    var out={ok:true,room_id:base.room_id||currentRoomId,
      owner_user_id:base.owner_user_id||(S.rest&&S.rest.owner_user_id)||window.__callOwnerUid||0,
      speakers:fill("speakers"), listeners:fill("listeners"), speaker_applicants:fill("speaker_applicants")};
    out.speaker_count=out.speakers.length; out.listener_count=out.listeners.length;
    return out;
  }catch(e){ return res }
}
function koeIsOwner(){
  try{
    ...
    var me=Number(window.__myUserId||0), ow=Number(window.__callOwnerUid||(window.__roomRoster&&window.__roomRoster.owner)||0);
    return !!(me&&ow&&me===ow);
  }catch(e){ return false }
}
...
if(typeof window.updateCallRoster==="function"&&!window.__koeRosterWrap){
  ...
  window.updateCallRoster=function(res){
    ...
    try{ renderApplicants(koeIsOwner()?(m.speaker_applicants||[]):[]) }catch(e){}
    return _ucr(m);
  };
}
...
function koeRoleOf(uid){
  ...
  var has=function(a){ return (a||[]).some(function(u){return Number(u.user_id||u.userId)===Number(uid)}) };
...
window.koeCallUserMenu=function(uid){
  uid=Number(uid); if(!uid)return;
  var me=Number(window.__myUserId||0), owner=koeIsOwner(), role=koeRoleOf(uid), nm=koeNameOf(uid);
  var items=[];
  if(owner&&uid!==me){
    if(role==="listener") items.push(["発言を依頼する",function(){ koeInviteToSpeak(uid,nm) }]);
    else if(role==="applicant"){
      items.push(["発言者にする",function(){ doApprove(uid) }]);
      items.push(["断る（リスナーに戻す）",function(){ doReject(uid) }]);
    } else if(role==="speaker") items.push(["リスナーに戻す",function(){ doReject(uid) }]);
  }
  if(uid!==me) items.push(["プロフィールを見る",function(){ try{viewProfile(uid)}catch(e){} }]);
  if(owner&&uid!==me) items.push([nm+" を退出させる",function(){ koeKickUser(uid,nm) }]);
  if(!items.length)return;
  var m=document.createElement("div"); m.className="modal"; m.style.display="flex";
  m.innerHTML='<div class="modal-content small"><div class="modal-header"><span>'+escapeHtml(nm)+'</span><button class="modal-close koe-um-x">✕</button></div>'
    +'<div class="modal-body" id="koeUmBody"></div></div>';
  document.body.appendChild(m);
  var close=function(){ try{m.remove()}catch(e){} };
  ...
  items.forEach(function(it){
    ...
    b.style.cssText="width:100%;margin-bottom:8px;"; b.textContent=it[0];
    b.addEventListener("click",function(){ close(); try{it[1]()}catch(e){} });
    body.appendChild(b);
  });
};
...
async function koeKickUser(uid,nm){
  ...
  toast((r&&r.ok)?(nm+" さんを退出させました"):("退出させられませんでした"+(r&&(r.message||r.error)?"："+(r.message||r.error):"")),(r&&r.ok)?undefined:"error");
  try{ refreshRoomStateNow() }catch(e){}
}

/* ---- 3) 発言の依頼（オーナー → 聞き専） ---- */
async function koeInviteToSpeak(uid,nm){
  if(!currentRoomId){ toast("枠に入っていません","error"); return; }
  if(!await showConfirmModal(nm+" さんに発言を依頼しますか？"))return;
  var r=await callApi("room_data_send",String(currentRoomId),"3",String(uid));
  if(r&&r.ok){ toast(nm+" さんに発言を依頼しました"); try{callLog("発言を依頼: "+nm)}catch(e){} }
  else{ toast("依頼を送れませんでした"+(r&&r.status?"（"+r.status+"）":""),"error"); }
}
...
window.koeHandleRoomData=function(raw){
  try{
    ...
    var key=JSON.stringify(d); if(key===window.__koeRoomDataSeen)return; window.__koeRoomDataSeen=key;
    var cmd=Number(d.command), rid=Number((d.args&&(d.args.requestee_id||d.args.requesteeId))||0);
    var me=Number(window.__myUserId||0);
    if(cmd===3){ if(rid&&rid===me) koeShowInvitePrompt(); }
    else if(cmd===4||cmd===5){
      if(!koeIsOwner()||!rid)return;
      var nm=koeNameOf(rid);
      if(cmd===4){ toast(nm+" さんが発言を承諾しました"); try{callLog("発言の依頼を承諾: "+nm)}catch(e){}
        callApi("approve_speaker",currentRoomId,String(rid)).then(function(){ try{refreshRoomStateNow()}catch(e){} }); }
      else { toast(nm+" さんに断られました"); try{callLog("発言の依頼を辞退: "+nm)}catch(e){} }
    }
  }catch(e){}
};
function koeShowInvitePrompt(){
  if(document.getElementById("koeInviteModal"))return;
  var m=document.createElement("div"); m.className="modal"; m.id="koeInviteModal"; m.style.display="flex";
  m.innerHTML='<div class="modal-content small"><div class="modal-header"><span>発言の依頼</span></div><div class="modal-body">'
  ...
  document.body.appendChild(m);
  try{sfx("join")}catch(e){} try{koeVibrate(400)}catch(e){}
  var close=function(){ try{m.remove()}catch(e){} };
  m.querySelector(".koe-iv-ok").addEventListener("click",function(){ close(); koeReplyInvite(true) });
  m.querySelector(".koe-iv-no").addEventListener("click",function(){ close(); koeReplyInvite(false) });
  setTimeout(function(){ if(document.getElementById("koeInviteModal")){ close(); koeReplyInvite(false) } },60000);
}
async function koeReplyInvite(ok){
  if(!currentRoomId)return;
  var me=Number(window.__myUserId||0); if(!me)return;
  var r=await callApi("room_data_send",String(currentRoomId),ok?"4":"5",String(me));
  if(ok) toast(r&&r.ok?"承諾しました。主催者が発言者にしてくれるのを待っています":"返事を送れませんでした",r&&r.ok?undefined:"error");
  else toast("辞退しました");
}
})();
...
(function(){
if(window.__koeRoomSize)return; window.__koeRoomSize=true;
function ct(){ try{ return localStorage.getItem("koe_room_ct")==="2"?2:1 }catch(e){ return 1 } }
window.koeRoomConnType=ct;
function paint(s){
  var row=document.getElementById("roomSizeRow"), d=document.getElementById("roomSizeDesc");
  if(!row)return;
  var cur=ct();
  ...
  if(d&&s){ d.textContent=(cur===1?s.sfu_desc:s.p2p_desc)||""; }
}
async function load(){
  var s=null;
  try{ var c=localStorage.getItem("koe_room_set"); if(c) s=JSON.parse(c); }catch(e){}
  paint(s);
  try{
    var r=await callApi("get_room_settings");
    if(r&&r.ok){ s={sfu_title:r.sfu_title,p2p_title:r.p2p_title,sfu_max:r.sfu_max,p2p_max:r.p2p_max,sfu_desc:r.sfu_desc,p2p_desc:r.p2p_desc};
      try{ localStorage.setItem("koe_room_set",JSON.stringify(s)) }catch(e){}
      paint(s); }
  }catch(e){}
}
function bind(){
  var row=document.getElementById("roomSizeRow"); if(!row||row.__b)return; row.__b=1;
  row.addEventListener("click",function(e){
    var b=e.target&&e.target.closest?e.target.closest(".room-size-chip"):null; if(!b)return;
    try{ localStorage.setItem("koe_room_ct",b.getAttribute("data-ct")) }catch(e){}
    try{ sfx("tab") }catch(e){}
    var s=null; try{ s=JSON.parse(localStorage.getItem("koe_room_set")||"null") }catch(e){}
    paint(s);
  });
  /* 枠作成の欄を開いたときに最新の設定を取りにいく */
  var btn=document.getElementById("openNewCallBtn");
  if(btn&&!btn.__rs){ btn.__rs=1; btn.addEventListener("click",function(){ setTimeout(load,50) }); }
  load();
}
...
(function(){
  ...
  window.doCreateRoom=function(){
    ...
    window.callApi=function(m){
      if(m==="create_room"){ var a=Array.prototype.slice.call(arguments); a[4]=ct(); window.callApi=_api; return _api.apply(this,a); }
      return _api.apply(this,arguments);
    };
    setTimeout(function(){ if(window.callApi!==_api) window.callApi=_api; },8000);
...
=============================================================================== */
(function(){
  if(window.__koeStackMgr)return; window.__koeStackMgr=true;
  var SEL=".modal,#imageLightbox,#dlFormatSheet,#imgFormatSheet,.koe-vol-backdrop";
  var BASE=3000, MAXZ=4650, next=BASE;
  function visible(el){
  ...
  function lift(el){
    try{
      ...
      next+=10; if(next>MAXZ)next=MAXZ;
  ...
  function scan(changed){
    ...
    for(var i=0;i<all.length;i++){
      var el=all[i];
      if(!visible(el)){onHidden(el);continue}
      if(!el.__koeShown){onShown(el);continue}
  ...
  try{
    var mo=new MutationObserver(function(recs){
      var need=false,changed=[];
      for(var i=0;i<recs.length;i++){
        var r=recs[i];
        if(r.type==="childList"){need=true;continue}
        var t=r.target;
        if(t&&t.matches&&t.matches(SEL)){need=true;if(changed.indexOf(t)<0)changed.push(t)}
      }
  ...
  window.__koeLiftNow=function(el){try{if(el){el.__koeShown=false;onShown(el)}}catch(e){}};
  ...
  if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",wrapOpeners)}else{wrapOpeners()}
  /* 後から差し替えられる関数(ブックマーク対応などで包み直される)にも追従する */
  setTimeout(wrapOpeners,1500); setTimeout(wrapOpeners,4000);
})();
...
(function(){
  if(window.__koeCardTap)return; window.__koeCardTap=true;
  var SKIP="a,button,input,textarea,select,audio,video,summary,label,"
