...
async function koeLoadMyQr(){
  var box=document.getElementById("koeQrBox"); if(!box) return;
  box.textContent="読み込み中…";
  var r=await callApi("get_my_qr_code");
  if(!r||r.ok===false){ box.innerHTML='<div class="empty-msg">'+escapeHtml(koeErrMsg(r))+'</div>'; return; }
  var url=r.qr_url||r.url||r.image_url||(r.qr&&(r.qr.url||r.qr.image_url))||"";
  var code=r.code||r.qr_code||"";
  box.innerHTML = (url?'<img src="'+escAttr(url)+'" alt="QR" style="max-width:220px;width:70%;border-radius:10px;background:#fff;padding:8px;">':"")
    + (code?'<div class="card-sub" style="margin-top:6px;word-break:break-all;">'+escapeHtml(code)+'</div>':"")
    + ((!url&&!code)?'<div class="empty-msg">QRコードを取得できませんでした</div>':"");
}
(function(){
  function bind(){
    ...
    var q=document.getElementById("koeQrSection");
    if(q&&!q.__b){ q.__b=1; q.addEventListener("toggle",function(){ if(this.open) koeLoadMyQr(); }); }
...
(function(){
  ...
  function gap(){ var idle=Date.now()-(window.__koeLastTouch||t0); return idle<60000?5000:20000; }
...
pixelify:{family:"'Pixelify Sans','Hiragino Sans','Noto Sans JP',sans-serif",url:"Pixelify+Sans:wght@400;700",label:"Pixelify Sans",note:"ゲーム風ドット・英数字だけ",g:"dot"},
silkscreen:{family:"'Silkscreen','Hiragino Sans','Noto Sans JP',sans-serif",url:"Silkscreen",label:"Silkscreen",note:"8bit風・英数字だけ",g:"dot"},
pressstart:{family:"'Press Start 2P','Hiragino Sans','Noto Sans JP',sans-serif",url:"Press+Start+2P",label:"Press Start 2P",note:"ファミコン風・英数字だけ",g:"dot"}
};
...
startLivePulse(),callApi("get_my_profile").then(r=>{r&&r.ok&&r.profile&&(myUserId=r.profile.user_id,window.__myUserId=r.profile.user_id,setTimeout(function(){try{koeAskSaveFolder(function(){setTimeout(function(){try{koeAskActOptIn()}catch(e){}},350)})}catch(e){try{koeAskActOptIn()}catch(_){}}},900))}).catch(()=>{})}async function doLogout(){await showConfirmModal("ログアウトしますか?")&&(await api().logout(),livePulseTimer&&(clearInterval(livePulseTimer),livePulseTimer=null),document.getElementById("loginEmail").value="",document.getElementById("loginPassword").value="",document.getElementById("loginError").textContent="",showScreen("loginScreen"))}async function downloadVoice(url){if(url&&/^https?:\/\//i.test(String(url))){if(window.AndroidApi&&window.AndroidApi.saveAudio)try{window.AndroidApi.saveAudio(url),toast("保存中…");try{haptic(10)}catch(e){}return}catch(e){}try{toast("ダウンロード中…");const res=await fetch(url),blob=await res.blob(),m=(url.split("?")[0].match(/\.[a-z0-9]{2,5}$/i)||[".m4a"])[0],a=document.createElement("a");a.href=URL.createObjectURL(blob),a.download="KoeTomo_"+Date.now()+m,document.body.appendChild(a),a.click(),setTimeout(()=>{try{URL.revokeObjectURL(a.href)}catch(e){}a.remove()},1500),toast("保存しました")}catch(e){toast("保存に失敗しました","error")}}}/* 保存形式は一度選んだら覚えておき、次からはタップだけで保存する。
...
function __koePdPend(text){ var id="p"+Date.now()+Math.floor(1000*Math.random());
  window.__pdPend.push({id:id,text:text,t:Date.now()}); __koePdSync(); return id; }
...
function __koePdSync(){
  ...
  box.__pdBusy=1;
  try{
    var body=box.innerText||"";
    list.slice().forEach(function(it){
      var has=!!box.querySelector('[data-pdpend="'+it.id+'"]');
      /* サーバー側の返信として出てきたら控えは消す */
      if(Date.now()-it.t>1500 && !has && body.indexOf(it.text)>=0){ window.__pdPend=window.__pdPend.filter(function(x){return x.id!==it.id}); return; }
      if(has) return;
