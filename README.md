...
<p align="center">
  <img alt="version" src="https://img.shields.io/badge/version-1.09-blue">
...
## 広告について（このアプリに広告が無い理由）

KoeTomo+ には広告を入れていません。「入れない」のではなく、**入れられない**理由があります。

- **広告ネットワークの規約に反する** — AdMob などの広告ネットワークは、他社サービスに無許可で接続するアプリ（非公式クライアント）での広告掲載を認めていません。掲載した時点で、このアプリも広告を出す側も規約違反になります。
- **公式の広告枠を借りることもできない** — 公式アプリが使っている広告アカウントの ID を別のアプリから使うと「不正トラフィック」と判定され、**運営側の広告アカウントが停止されるおそれ**があります。応援したい相手に損害を与える行為になるため、絶対に行いません。
- **非営利の方針と矛盾する** — このアプリは無償で配布し、[LICENSE](LICENSE) でも商用利用を禁止しています。開発者はこのアプリから一切の収益を得ていません。

### それでも koetomo を応援したい方へ

運営に収入が届く方法は、**公式の仕組みを使うこと**です。KoeTomo+ の通話画面とマイページにある「公式オファーウォール」、および公式アプリからのコイン購入は、いずれも運営（株式会社ミーツ）の売上になります。KoeTomo+ はそこへの入口を用意するだけで、間に入って収益を受け取ることはありません。

このアプリは「koetomo が続いてほしい」という気持ちで作られています。運営から要請があれば、いつでも配布を停止します。
